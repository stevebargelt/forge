...
## Demoted to idea 2026-07-25 — known limitation, not current work

Relative-pointer projects (submodules, `git worktree add --relative-paths` on git 2.48+) fail LOUDLY at the container-side probe with exit 122 and a named diagnostic, which satisfies FG-559's safety requirement that silent degradation is impossible. A characterization test pins the behavior so it cannot regress into silence, and `docs/concepts.md` documents the scope limit honestly.

Submodule and future-git support is not a current blocker for any workflow forge runs. Promote back to a story only when a real project needs it.
