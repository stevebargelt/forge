...
3. Campaign-7a56519b2f3d cleanup (non-ticket thread, operator go STILL pending): items stuck failed/campaign_system — recoverable via forge campaign reconcile (delivered items) or FG-511 evidence-gated forge campaign retry (transient items). First dogfood of both paths.
4. FG-477 (lifecycle evaluator): body now records FG-512+FG-519 as shipped narrowing slices; the evaluator should ABSORB resolvePhasePrimary, not re-derive it. Slice 1 (classifier) is the dispatch-ready next move there.
