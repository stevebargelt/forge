...
- **Platform gate (DECIDED):** macOS only. **Hard-fail on Linux** with a clear message unless `FORGE_NO_WORKTREES=1`. Do not silently run a known-broken Linux path (node_modules gap → confusing agent failures). Linux support is FG-358.  *(Superseded by FG-345, 2026-07-28: FG-358 was closed OUT-OF-SCOPE on 2026-07-02 — the orchestrator runs only on a macOS host per DEC-004, and "Linux" in these gates means the agent CONTAINER — so the Linux hard-fail is PERMANENT by design, not pending. Worktree mode is also no longer behind an explicit gate: it is the default on darwin, with `FORGE_NO_WORKTREES=1` as the escape.)*
