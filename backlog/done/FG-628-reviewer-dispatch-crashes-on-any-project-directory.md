...
- **`(A4)` skips without a reachable probe image** — though not on this host. It defaults to `busybox:latest`, and its guard falls back from `docker image inspect` to `docker pull`, so it self-certifies wherever a daemon and the registry are reachable. Re-verified 2026-07-28 with NO env override: 7/7 pass, `skipped 0`. The earlier capture used `FORGE_TEST_DOCKER_PROBE_IMAGE=ubuntu:22.04` because a wedged `docker-credential-desktop` was hanging every pull with zero output — the docker CLI invokes that helper before printing anything, so it read as a network block when it was a local IPC hang; restarting Docker Desktop cleared it. The limit still holds for a host with a daemon but no reachable image: that gets a loud skip, never a pass.
