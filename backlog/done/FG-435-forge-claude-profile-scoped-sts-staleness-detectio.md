...
> **Superseded in part by FG-499 (2026-07-08):** the interactive `forge claude` preflight no longer hard-blocks on ANY credential staleness/expiry — both former hard-block shapes are advisory warnings (same profile-named, timezone-labeled diagnostics) and launch proceeds; native claude/AWS auth owns interactive failure. The container-dispatch gate (validateCredsForNewRun) keeps blocking. AC below describing hard-block messages reflects FG-435 as shipped, before FG-499.
