...
**Root cause (hypothesis):** The maintainer seed treats `CLAUDE.md` as off-limits, consistent with forge's rule that "re-rendering CLAUDE.md via `forge-dev upgrade` and marker-repair are deterministic, not authoring" (the rule named plain `forge upgrade` until FG-577 split stable/dev — see below). But that rule is about the RENDERED orchestrator block (between the forge markers). The repo-root CLAUDE.md ALSO contains hand-authored, forge-repo-specific prose ABOVE the marker block — the "## File layout" tree, conventions, auth-mode notes, etc. That hand-authored region has NO owner:
- the documentation-maintainer declines the whole file, and
- `forge-dev upgrade` only re-renders the marker block, never the hand-authored top. (Since FG-577, stable
  `forge upgrade` renders the EXECUTING release's template, so seed edits in the checkout propagate only via
  `forge-dev upgrade`. The region boundary described here is unchanged; only which forge you run changed.)
