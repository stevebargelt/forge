...
Surfaced live by FG-425's cross-process e2e regression (src/v2/fg425-project-gate-lock.worktree.test.ts): ~1 in 4 runs, `SqliteError: database is locked` at runs.ts:148 in one of two concurrently-finalizing drivers. reconcile.ts already documents this class ("A DB-layer throw (SQLITE_BUSY) when two forge processes reconcile the same…") and guards; run-finalize does not.
...
- [ ] Two processes concurrently finalizing different runs on one DB never crash with SQLITE_BUSY (regression: drive the FG-425 cross-process harness without its retry shim)
