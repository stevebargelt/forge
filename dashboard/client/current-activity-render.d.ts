...
/** Did the submitter declare `--run` / `--task` / `--ticket`? Read off the ids, not off
 *  the `unassociated` label, so an omitted label cannot promote a cwd-only launch. */
export function launchIsAssociatedWait(entry: Partial<HostLaunchEntry> | null | undefined): boolean;
/** Associated AND observed AND running. A stale row is not evidence about now, a
 *  terminal one is history, and an unassociated one is host activity nobody is waiting
 *  on. All three still render in full on the diagnostic surfaces. */
