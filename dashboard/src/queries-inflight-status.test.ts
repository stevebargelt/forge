// FG-521 (d): `inFlight` used to select 'awaiting_human_input' — a status that
// is not in TaskStatus and that nothing ever writes. Dropping it must be a
// no-op: the in-flight set is exactly {running, awaiting_gate, awaiting_red,
...
// was removed alongside it, and any status inFlight CAN return must still have
// one to render.
//
...
// The status queries.ts used to select but that TaskStatus has never contained.
...

{
  ...
  // The phantom the query used to select — nothing writes it, but if a legacy
  // row ever carried it, it is NOT a status the dashboard knows how to render.
  insert.run(`task-${PHANTOM_STATUS}`, "run-active", "engineer", "engineer", "sonnet", PHANTOM_STATUS, null, "2026-07-11T10:00:00Z");
...
test("inFlight: the phantom 'awaiting_human_input' row is not returned — dropping it from the query changed nothing", () => {
  const ids = new Set(inFlight(undefined, probe).map((r) => r.taskId));
  assert.equal(ids.has(`task-${PHANTOM_STATUS}`), false);

  // And the row is still there — the read is a filter, not a cleanup.
  const db = new Database(join(tmpHome, "forge.db"), { readonly: true });
  const row = db.prepare("SELECT status FROM tasks WHERE id = ?").get(`task-${PHANTOM_STATUS}`) as { status: string };
  db.close();
  assert.equal(row.status, PHANTOM_STATUS);
});
...
test("shell: every status inFlight can return still has a badge rule — and the phantom's rule is gone", () => {
  ...
  assert.doesNotMatch(shell, new RegExp(`\\.badge\\.status-${PHANTOM_STATUS}\\b`));
});
