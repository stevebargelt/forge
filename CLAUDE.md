...
5. **Run complete (FG-474):** Before calling a pipeline run complete, confirm the required CI check (`CI / test` — `.github/workflows/ci.yml`) is green for the head commit — `gh pr checks <pr>` or `gh run list --commit <sha> --workflow CI --json conclusion`. Do NOT re-run `npm run test:all` on the host yourself: CI already ran the full shipped-claim aggregate (root suite + dashboard workspace) off-host, once, visibly — a second host run only reproduces what CI already proved. If nothing has been pushed yet (so no CI run exists for the commit), push first and wait for CI rather than substituting a host run. Then summarize what shipped, what each phase produced, follow-ups worth filing via `forge backlog file`.
...
- **Present before you start the loop:** ticket id, route key, commit range (or `--since`), max rounds, the reviewer/fixer roles (`red-wide` read-only / `engineer`), the verification commands, the required CI check (`CI / test` — `.github/workflows/ci.yml`), and the stop conditions. (`forge review-loop … --dry-run` prints most of this.)
...
**Merge authorization (FG-436, tightened by FG-474).** A passing review-loop (`closeable` — reviewer pass AND the loop's own host verification green) **plus** the required CI check green (`CI / test` — `.github/workflows/ci.yml`, which runs `npm ci`, the better-sqlite3 native rebuild, `npm run typecheck`, and `npm run test:all` off-host on the pushed branch/PR) is **sufficient authorization to merge the PR** — you do not wait for a separate human PR review after all required automated gates pass, and you do NOT re-run `npm run test:all` on the host yourself before merging (FG-474): the CI check IS the deterministic-verification gate for merge now, not a host re-run of the same suite the loop and CI already ran. Present the range, roles, max rounds, verification, required CI, and stop conditions first; then, on a clean pass, merge. **Auto-merge is BLOCKED — do not merge, surface the reason — when any required condition is absent or failing:** the review-loop exhausted its rounds without a pass; the review-loop's own verification failed or is missing; the required CI check is not green (not yet finished, absent, or failed); an unresolved blocking reviewer finding remains (per the disposition policy above); the branch is dirty, unpushed, has a merge conflict, or is stale behind base and must be updated first. This authorization covers only code/durable-behavior implementation work — it does NOT bypass branch protection or required CI, and it does NOT extend to changing routing/governance or the operator-policy surfaces (those keep their own confirm-before-acting gate).

**CI vs review-loop verification — not the same run duplicated (FG-474).** The review-loop's host verification (typecheck + root `test`, via `runVerification` in `src/v2/review-loop.ts`) is the loop's OWN gate for reviewer/fixer dispatch decisions and runs on the local branch during the loop — it stays as-is; the engine's "host verification" mechanism is a locked MVP decision (#301) and is generic across any project `review-loop` targets, not all of which have CI. The CI check (`CI / test`) is the separate MERGE gate: it runs the full aggregate (root + dashboard workspace) off-host, once, visibly, on the pushed branch/PR. What FG-474 removed is the THIRD run — the orchestrator's own extra `npm run test:all` re-run of the same commit immediately before merging, which reproduced what CI (and the loop) had already proven and was invisible in the dashboard while it ran. Reusing CI's result inside the review-loop itself (so its own host run could be skipped when a covering CI result already exists) is a further, structurally bigger change — deferred as a follow-up, not done here.

**Branch protection on `main`.** Requiring the `CI / test` check on `main` is a one-time GitHub setting — `gh api repos/<owner>/<repo>/branches/main/protection -X PUT -f required_status_checks[strict]=true -f 'required_status_checks[contexts][]=test'` (or Settings → Branches → main → required status checks → `test`) — run once with real GitHub admin credentials. This is shared-repo configuration, not something an agent container (no GitHub credentials, no push/PR access by design) can or should apply; do it from the host under your own GitHub session.

## Before closing a backlog ticket

This is the single closing gate. A ticket closes ONLY when **every one of its acceptance criteria is met, with evidence.** The checks scattered above (implementer validation, gate-decision discipline, the docs-impact lifecycle, review-loop `closeable`) feed this gate — they are necessary but **not sufficient** on their own. The required CI check (`CI / test`) green proves the suite passes; it does NOT prove the AC.
