...
5. **One batch fix.** Send ALL current `fix_now` findings to ONE fixer in ONE immutable, candidate-bound FixBatch naming each finding ID with its evidence. Not one fixer per round; not fix-as-you-go. A changed disposition creates a new batch REVISION rather than mutating the running task's inputs.
...
- **A `fixer scope guard — reverted disallowed paths` note is FYI, not a stop-and-ask (FG-502).** Paths the fixer touched outside the reviewed range were selectively reverted before commit, with a reason for each. Informational only: the revert already happened, the in-scope fix still landed, and no extra cycle is needed. Read the listed paths and reasons (they often flag genuine docs drift filed as a follow-up) so the guidance isn't silently dropped.
