...
- **Wait completion-driven — a launch-completion wake advances the transition; `ScheduleWakeup` is ONLY a lost-signal watchdog, never a job-duration timer (FG-563 / BD-9).** Sizing a wakeup "matched to how long the work takes" is a defect, not the wait path — the work signals its own completion, and no `sleep N` in Bash ever waits on launched work. The division of labor: **tmux (`forge launch run`) owns the work, `forge launch wait` observes its completion, `forge continue` claims and advances the transition, and `ScheduleWakeup` is only a fixed health-bound backstop for a lost wake.** For every long transition:
  ...
  2. **Arm the wake:** a disposable, single-shot `Monitor` whose command is exactly one blocking `forge launch wait <launch-id> --json`. That call blocks until a terminal disposition — exit 0, ordinary non-zero, OS signal, owner-gone, unknown, or persistently-unreadable — and emits ONE structured observation; that line is your wake, delivered with no operator turn. `Monitor` is a DEFERRED harness tool — it is NOT callable until you load it with `ToolSearch("select:Monitor")`, so a session that never loaded it will report it unavailable even when it is present; always ToolSearch it before concluding anything. It is the OPPORTUNISTIC fast-path, NOT a guaranteed one: in some environments it is genuinely absent (a client that does not provide it, or a session with background tasks disabled), and you must not depend on it. (`forge claude` sets `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1` on every orchestrator session — claude-adapter.ts, FG-535 — to force agents through `forge launch run` rather than reapable Bash background dispatch; whether that flag ALSO removes the Monitor tool is client-dependent and NOT assumed here — confirm with ToolSearch on the host in question rather than inferring.) **If `Monitor` will not load or run, that is the newer-client case, NOT a dead end — your `ScheduleWakeup` (step 3) IS the wake:** it fires generally as a delayed self-re-invocation carrying its prompt (the `/loop` framing in its schema is not a limit — this whole pattern relies on it), so on that wake you POLL durable forge state (`forge launch show <id>` / `forge status`) and advance. Net: Monitor present → it wakes you and ScheduleWakeup backstops; Monitor absent → ScheduleWakeup wakes you and you poll. Either way a single blocking/scheduled wait, NOT a repeating poll of `forge launch show` held open in one turn, and never a `sleep`/`while` loop or a blocking foreground wait.
