...
**TEMPORARY OPERATING POLICY** — activated from `docs/prds/evidence-led-review-lifecycle.md` (Change 0), in force until the evidence-led lifecycle becomes the DEFAULT (Change 3 / FG-640). It SUPERSEDES the former #301/FG-436 repeated-loop policy: no `--max-rounds 2` default, no rerunning the loop until the reviewer passes, no fixer-per-round remediation, and a reviewer pass is never, by itself, evidence that earlier findings were resolved. This changes how model review is CONDUCTED — every deterministic and authority gate is preserved unchanged (rule 11 below). **The conduct below is unchanged by Changes 1 and 2 having shipped, and Change 0 is NOT retired.** FG-638 added the durable `reviews`/`review_findings` ledger and the `forge review show` / `forge review disposition` surfaces over it; FG-639 added the coordinator, so `forge review start <ticket-id> --contract <file>` DOES now open a review and ingest findings, and `forge review continue <review-id>` drives it from persisted state. That coordinator is an explicit PILOT and nothing more: the `feature` workflow is NOT migrated, so no ordinary run opens a review of its own accord, and NO gate authority changed — the `review_disposition` gate derivation is still Change 3 (FG-640). So for any review you are not deliberately driving through `forge review start`, everything below applies exactly as written and the finding ledger in step 3 is yours to author. When you DO drive one through the pilot, the coordinator owns the ledger and the staging — the rules below are what it enforces, not a second process to run alongside it.
