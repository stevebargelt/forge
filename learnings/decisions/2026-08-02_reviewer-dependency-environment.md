...
- Cache identity and engine identity are recorded as ONE fact in three places — the task manifest (authoritative), the review's recheck stage evidence, and the event timeline — so "which dependency environment produced this evidence" is read back from durable state instead of inferred. The timeline is what makes the SUCCESSFUL case auditable: a manifest is a file beside one dispatch, and the question an audit asks is "which engine did this verification run against", which is answered by `container.dependency_environment_resolved` on both read-only lanes. Refusals were already durable there (`container.dependency_provisioning_failed`, `stage: "environment_resolution"`).
...
- **The timeline event names its fields; it never spreads the receipt.** `container.dependency_environment_resolved`'s payload is a closed set (`stage`, `cacheKey`, `probeImage`, `nodeVersion`, `abi`, and per package `name`/`version`/`loaded`), built field by field from the receipt. The receipt's `artifact`, `resolvedPath` and `staleCacheRepaired.disprovenRoot` are host paths and the events table is read far from the dispatch that wrote it, so a spread would let any field added to the receipt later reach the ledger by default. Widening the payload is a decision to be made once per field, not a consequence of editing the receipt.
- **No schema change, no migration.** `blocked_environment`, `COVERAGE_OUTCOMES` and `environment_blocked` on regression_test evidence all already existed; on the review plane FG-664 adds a PRODUCER, not vocabulary. The one name it does mint is the timeline's `container.dependency_environment_resolved`, and `events.event_type` is an unconstrained `TEXT` column, so that costs no DDL and no migration either — a new event type is a new value, not a new shape. A migration to `~/.forge/forge.db` re-runs in every live forge process on next DB open — that cost is not worth paying for a value the schema already accepts.
