...
**Addendum (review fix batch `fix-batch-fa188d5044fd`, 2026-09-08): RF-1 and RF-2.** The evidence-led review of this change surfaced two gaps in the AC3 preflight as first landed:

- **RF-1 — the preflight's refusal leaked its lane entry.** `recordPublicationIntent` enqueues a `queued` publication-lane row before the preflight runs; the preflight originally sat *before* the `try`/`finally` that releases it via `leaveLane`, so a refusal returned without ever entering that `finally` and left the row live — queuing every later publication for the run behind a phantom. The fix moves the preflight inside the `try`, ahead of `awaitLaneTurn`, so a refusal now returns through the same `finally` and the lane is empty afterward. This does not change what a refusal reports or when it fires relative to the worktree/mutex/ref work described above — only that it now also cleans up after itself.
- **RF-2 — a settled candidate with an unrecorded phase base used to fail OPEN.** The original ancestry check (`phaseBase && currentCandidate && ...`) required a truthy `phaseBase` before it could refuse, so a task with no recorded `base_sha` at all published unchecked under a settled review — the exact unknown-provenance case AC3 exists to catch. `publishIntegration` now checks `settledCandidate` (`latestSettledReviewCandidateForRun`, not the broader `currentCandidate` used by the ancestry check — a publication-receipt-only candidate legitimately has no base yet, e.g. the build fan-out's parent task) against a missing `phaseBase` FIRST, and refuses with its own reason, `base_unrecorded_under_settled_candidate`, and its own message (`describeUnrecordedBaseRefusal` in `src/v2/project-identity.ts`). The pre-settlement / legacy no-candidate path is unaffected: a missing base with no settled candidate to be unknown-provenance against still proceeds exactly as before.

Both are covered by regression tests in `src/v2/fg791-stale-base-publish-refusal.worktree.test.ts` (the RF-1 and RF-2 test blocks). The docs and schema contract (`docs/concepts.md`, `docs/SCHEMA-CONTRACT.md`, `docs/how-to-new-feature.md`) now name both `publication.refused` reasons.
