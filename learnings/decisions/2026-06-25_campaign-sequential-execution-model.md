...
- **FG-737 fixed a detached-resume gap in that same lease model** (2026-08-20): a campaign item HELD on the first controller (a settled drive-item linkage, `run_id IS NULL`) never recovered on a detached `forge campaign resume`, because the drive-item born-under fence stayed pinned to the first controller's instance-unique `cli-<pid>` owner and every later resume ran under a *different* instance-unique owner — the fence denied forever. The fix does not weaken FG-564's owner-uniqueness (a stable `@auto` fallback was considered and rejected — it would let two concurrent same-campaign controllers share an owner, both renew at the same generation, and both pass the born-under fence). Instead, `recordDriveItemLaunchLinkage` now refreshes a *settled* linkage's born-under token to the current controller's owner/generation on re-record (guarded on `run_id IS NULL`, and on the caller still holding the live lease, so a stale launcher that lost a takeover race cannot rebind it) — a genuinely in-flight linkage is left untouched. See docs/concepts.md (Crash recovery).
