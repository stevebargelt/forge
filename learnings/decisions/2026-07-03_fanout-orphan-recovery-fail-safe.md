...
>
> **Further amended by FG-688 (2026-08-10).** Closes the gap FG-584 left open: an ordered wave that reaches a *terminal* failure (`prerequisite_blocked` above all, plus `integration_blocked` and the mid-wave no-verdict arms) is no longer stuck re-running completed work either. `--re-drive` on such a parent now reopens the **same parent row** in place (`failed` → `pending`, children kept attached) instead of minting a fresh one, so the wave's existing adoption logic — recomputed from durable state, same as the crash-resume path — adopts every item whose captured commit is already integrated and dispatches only what didn't complete. This resolves this decision's "Negative / Trade-offs" bullet and the first "Revisit Condition" below, but only for the **ordered** shape, and not by the `dispatchFanoutStep` child-subset surgery those sections anticipated — reusing the parent row made that surgery unnecessary. The **unordered** lane is untouched: `--re-drive` still mints one fresh pending primary and still re-runs the full wave, so that trade-off stands as written. FG-688 also fixed a related defect found alongside it: the read-only inspector recommended `--re-drive` for every fanout parent regardless of failure kind, so on a `prerequisite_blocked` parent following forge's own advice produced a refusal; the recommendation now routes through the same eligibility predicate the mutation guard enforces. See [Adopt-preserving re-drive of an ordered wave](../../docs/concepts.md#adopt-preserving-re-drive-of-an-ordered-wave).
>
> **Further amended by FG-527 (2026-08-15).** The `forge retry` fanout-child discriminator this decision's retry-side closure depends on (`fanoutParentOf`, Implementation Notes below) no longer guesses from the agent's role name. It used to treat a parented row as a genuine fanout child whenever its phase matched its would-be parent's AND its agent role lacked a `red-` prefix — which misread a red reviewer declared without that prefix (feature.yml's non-`red-`-prefixed `shipping-reviewer`) as a fanout child and refused its retry with `FanoutChildRetryError`, pointing the operator at the wrong recovery (`forge recover <parent> --re-drive`) for a task that was never part of a wave. `fanoutParentOf` now loads the run's workflow and asks the FG-477 lineage classifier (`classifyTaskLineage`) whether the row is `fanout_child` or `red_review`; a `shipping-reviewer`-style red classifies `red_review` and retries directly, while a genuine fanout child still classifies `fanout_child` and is still refused exactly as this decision describes. **When the run's workflow itself cannot be loaded** (missing, unparseable, or an unresolvable workflow name), `fanoutParentOf` does not fail open by skipping the check: it classifies against a degraded, empty-steps workflow instead, which resolves any parented non-recovery row to `fanout_child`, so `forge retry` refuses it (`--force` still overrides) rather than risk retrying an actual fanout child it simply couldn't prove. This over-refuses a `red_review` whose workflow won't load — the correct direction for a structural guard, since a wrongly-permitted fanout-child retry mints a stray primary and corrupts the wave, while a wrongly-refused retry is one `--force` away. Deliberate operator-visible retry behavior change, scoped to this one call site and to `runNext.ts`'s `dispatchFanoutStep` child filters (same classifier, same replacement of the `red-` prefix heuristic); `recover.ts`, `reconcile.ts`, `executor.ts` and `resolvePhasePrimary` still use the pre-FG-477 heuristics as later slices of that migration.
>
> **Further amended by FG-716 (2026-08-15).** Closes two of the four call sites the FG-527 amendment above left on pre-FG-477 heuristics — and fixes a real misclassification along the way. `recover.ts`'s `isFanoutParent` used to treat a parented row with **any** child at all as proof of a fanout wave; a plain phase step whose only child was its red reviewer (no wave, no `dispatchFanoutChild` involvement) was therefore misidentified as a fanout parent — `forge recover <id>` returned the wrong `FanoutParentView` shape for it instead of ordinary task evidence, and it could wrongly appear in a run-level `forge recover <runId>`'s `fanoutParents` listing. It now requires at least one child satisfying the new `isFanoutChildRow` primitive (`lifecycle-evaluator.ts`), which reads the `fanoutIndex` marker `dispatchFanoutChild` stamps (FG-584 D6) without loading the run's workflow — a property test pins it as exactly the FG-477 classifier's `fanout_child` cell over the generated corpus, including the one shape production can't reach (a parented `invoke` row carrying `fanoutIndex`, which rule 0's `dispatchSource === "invoke"` precedence still calls `adhoc_invoke`, so the primitive must reject it too). `reconcile.ts`'s `awaiting_red` sweep is switched onto the same primitive in place of its own inline `fanoutIndex` check; its outcome there is unchanged since every child it inspects there is already parented and non-recovery. `executor.ts` and `resolvePhasePrimary` remain on their pre-FG-477 heuristics — the latter by deliberate migration-freeze (see the comment above `resolveCompletedPhasePrimary` in `lifecycle-evaluator.ts`, as of FG-717 below), not oversight.
>
> **Further amended by FG-717 (2026-08-15).** Pure relocation, no behavior change to the heuristic itself. The canonical selection rule and its migration-freeze rationale (referenced by the FG-527 and FG-716 amendments above) moved out of `ready-queue.ts`'s `resolvePhasePrimary` into `lifecycle-evaluator.ts` as the exported `resolveCompletedPhasePrimary(candidateRows, phase)`; `resolvePhasePrimary` is now a thin alias of it, kept so `ready-queue.ts`'s own call sites (`computeReadyQueue`'s phase-closed and dep-satisfied checks) read unchanged. `inputs.ts`'s `deriveUpstream` and `runNext.ts`'s fanout upstream read now import `resolveCompletedPhasePrimary` directly rather than going through the alias. Still selection-only over whatever row universe it's handed — no provenance/policy filtering, no classifier — so `executor.ts` and this function both remain on the pre-FG-477 heuristic exactly as the FG-716 amendment describes; only the file it lives in changed.
>
> **Further amended by FG-718 (2026-08-15).** Overtakes the "kept so `ready-queue.ts`'s own call sites... read unchanged" clause of the FG-717 amendment just above: those call sites have themselves now moved. `ready-queue.ts`'s private lifecycle derivations — the ready-queue computation, per-step settle states, the run-settled reachability check, and the terminal classifier — are consolidated into one pure snapshot provider, `evaluateLifecycle(workflow, tasks)` in `lifecycle-evaluator.ts`, which calls `resolveCompletedPhasePrimary` directly rather than through the `resolvePhasePrimary` alias. `computeReadyQueue`'s phase-closed and dep-satisfied checks referenced above now live in that provider's internal `computeReadyWork`, not in `ready-queue.ts`. `ready-queue.ts`'s three public functions (`computeReadyQueue`, `isRunSettled`, `classifyRunTerminalState`) and the `resolvePhasePrimary` alias are unchanged in signature and behavior — each is now a thin projection of one `evaluateLifecycle` snapshot instead of its own independent derivation — so nothing above this note about caller-visible behavior needs correction, only the internal call-site location. Pure consolidation, no behavior change; repointing `recover.ts`/`retry.ts`/`reconcile.ts`/`executor.ts` callers onto the snapshot directly (closing the remaining pre-FG-477 heuristic gaps the FG-716/FG-717 amendments track) is later FG-477 work, not this slice.
>
> **Further amended by FG-720 (2026-08-15).** Closes `gate.ts`'s own fanout-parent derivation — the last call site anywhere in the codebase still deciding fanout-parent-ness from the retired `inputs.fanout === "object"` task marker rather than from the FG-477 evaluator. The advance branch now calls a new `isFanoutParentRow(task, step)` (`lifecycle-evaluator.ts`): a phase-primary row (`isPhasePrimaryRow`) whose workflow **step** declares `fanout`, derived from the workflow declaration rather than a task-side marker. The derivation is byte-identical on every gate-reachable row: the marker was stamped only on the parent primary `dispatchFanoutStep` mints, and the one fanout-step primary that lacks it — the defensive-failure parent minted with an empty task package — is `failTask`'d before it can reach a gateable status, so it never reaches this branch either way. No behavior change. `aggregateVerdicts` / `verdictBlocksGate`, the shared canonical gate-blocking predicate, are a separate, deliberately untouched part of `gate.ts`'s decision surface.
>
> **Further amended by FG-721 (2026-08-15).** Closes `campaign/executor.ts`'s `reconcileTerminalOutcome` unresolved-outcome fallback — the last of the `recover.ts`/`retry.ts`/`reconcile.ts`/`executor.ts` call sites the FG-718 amendment named as remaining pre-FG-477 heuristic gaps (this one specifically; the `campaign_system` retry-eligibility probe elsewhere in the same file is a separate, still-untouched ad-hoc scan). The fallback used to derive its failed-primary set by scanning every top-level failed task row the run ever recorded (`parentId === undefined && status === 'failed'`); it now selects from `classifyRunTerminalState(workflow, tasks).failedPhases` (the FG-718 projection over `evaluateLifecycle().terminal`) — phases whose own primaries terminally failed with no complete replacement — then classifies every terminally-failed primary attempt within each failed phase (via `isPhasePrimaryRow`, not a re-derived parentId scan) through the same `failureKindForTask` -> `classifyFailureKind` sequence, keeping the FailureKind -> BlockerKind translation and SHARED-WINS precedence in `src/campaign` (the evaluator returns campaign-neutral step ids, never a BlockerKind). Two shapes the old scan wrongly counted are now excluded: a SUPERSEDED failed primary (an earlier attempt failed but a later attempt completed the same phase — the request-changes shape) and a failed AD-HOC invoke row (never a workflow phase) — neither now escalates an item to the whole-campaign-pause `campaign_system` default. Operator-visible behavior change, scoped to this one fallback: an item whose run's only failed evidence was a superseded attempt or an unrelated ad-hoc invoke failure now resolves to the correct LOCAL blocker instead of incorrectly pausing the whole campaign. See [Outcome semantics](../../docs/concepts.md#start-sequential-execution).
...
- `src/v2/retry.ts` — `FanoutChildRetryError` + `fanoutParentOf` (since FG-527, loads the run's workflow and asks the FG-477 `classifyTaskLineage` classifier whether the row is `fanout_child`, failing closed to a degraded empty-steps workflow — and so to `fanout_child` — when the load itself fails; originally phase-equality plus a non-`red-` agent-role-prefix guess, which misclassified a non-`red-`-prefixed red reviewer as a fanout child — see the FG-527 amendment above).
