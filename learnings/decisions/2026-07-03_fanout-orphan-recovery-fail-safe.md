...
- `src/v2/retry.ts` — `FanoutChildRetryError` + `fanoutParentOf` (since FG-527, loads the run's workflow and asks the FG-477 `classifyTaskLineage` classifier whether the row is `fanout_child`; originally phase-equality plus a non-`red-` agent-role-prefix guess, which misclassified a non-`red-`-prefixed red reviewer as a fanout child — see the FG-527 amendment above).
