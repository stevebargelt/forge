...
- **`forge publish recover <attemptId>`** runs the same idempotent convergence by hand. It takes the publication window or refuses, naming the holder.
...
- **`reconcilePublicationRecoveries` → `setTaskStatus`** (the `failed` → `awaiting_recovery` repair step). It clears a terminal claim left standing over an attempt AD-5 recovery has since converged to `published`, so that `finalizePrimary`'s completion CAS — which refuses to overwrite a `failed` row *on purpose*, guarding a completing container against a concurrent cancel — can land it. The repair steps **through** `awaiting_recovery` rather than around it, so that guard is never relaxed. Idempotent and derived entirely from the durable attempt record: a crash on either side leaves the same record for the next wave to re-derive the same repair from.
- **`reconcilePublicationRecoveries` → `failTask`** (plus its `task.publication_reconciled` audit event). The terminal failure for a task whose attempt converged to a **non-published** disposition — the ref provably does not carry its candidate, so nothing of it was published. A crash before this write leaves the task non-terminal with the attempt already settled, and the next wave re-derives exactly this landing from that same settled record.
