...
`forge dashboard start --remote` opts in to the **Remote Board** (FG-781): an off-by-default, read-only, project-scoped second listener on its own loopback port (`:8025`). It refuses every request until a transport adapter is selected: a tailnet-private Tailscale Serve proxy (FG-782, `FORGE_DASHBOARD_REMOTE_TRANSPORT=tailscale`) or a public-hostname Cloudflare Tunnel gated by Cloudflare Access (FG-784, `FORGE_DASHBOARD_REMOTE_TRANSPORT=cloudflare`). See `dashboard/README.md` → "Remote Board".
