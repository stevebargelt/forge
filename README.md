...
Because seed installation is release-owned, `forge upgrade` repairs a drifted `~/.forge/` — its forge-owned seeds, at least — on a machine that has never cloned forge. Operator-authored seeds (agents, constraints, `forge-raci.md`) that have diverged are **retained, not repaired**: forge reports them and leaves the merge to you. The Forge-owned agent protocols are repaired rather than retained, because they are not in your files: a protocol that is missing, torn, or behind the running release fails `forge doctor` and makes the review lifecycle's roles refuse to dispatch until `forge upgrade` republishes the generation. Two protocol states upgrade cannot repair, and re-running it changes nothing for either: an embedded legacy copy left inside an agent seed of yours — doctor names the file and the section, and the deletion is yours — and a release that itself carries no protocol for a covered role, which is the executing tree's problem rather than this host's, so reinstall the release; publication refuses that same missing file.
