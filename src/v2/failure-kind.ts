...
export type FailureKind =
  ...
  | "fanout_wave_orphaned"     // FG-455 p2: an UNORDERED fanout PARENT left `running` after its children finished or died mid-wave with the process gone — reconciled from child evidence; not every child completed, so the parent fails pointing the operator at `forge recover <parent> --re-drive` (forge retry refuses this kind; forge show recommends the same). FG-584 narrowed the scope: an ORDERED wave's stranded parent does NOT land here. Its prerequisites are typically already captured, merged and gated, so reconcile returns it to `pending` and the controller RESUMES it — adopting captured work rather than re-running it — which is why this kind stayed an operator-driven re-drive and did not grow a resume arm. FG-584 RF-3 completed that narrowing: an ordered wave never lands here on ANY arm, including the live-lock-holder one, where it is simply left running for the process that holds the run — a re-drive there mints a fresh parent that cannot adopt the captured children and re-runs them
