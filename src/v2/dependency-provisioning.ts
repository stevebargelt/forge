...
// node_modules).
//
// FG-806: ONE renderer for both dispatch lanes — invoke and runNext emit this
// section byte-for-byte identically, gated on the same fact: the resolver
// returned `ready` and handed back a receipt.
export function renderDependencyEnvironmentSection(receipt: DependencyEnvironmentReceipt | undefined): string[] {
  if (!receipt) return [];
  return [
