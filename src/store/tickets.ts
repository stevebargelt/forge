...
// ─── reconcile (FG-606): keep the shadow EQUAL to the current Markdown set ────
// A re-import after a Markdown REMOVAL/RENAME must leave no stale rows behind, or
// the shadow drifts from "DB rows equal the Markdown set". These run INSIDE the
// import's write transaction. Removal is PROVENANCE-scoped by importedFrom (the
// canonical source dir): two linked worktrees share a project_key but have
// different Markdown sets, so a removal from ONE worktree must never prune a
// SIBLING worktree's tickets. Other projects' rows are never touched. id_sequence
// is deliberately NOT reconciled — it is a monotonic high-water mark; removing the
// highest ticket must never let its id be reused.
export function pruneRemovedTickets(
  projectKey: string,
  importedFrom: string,
  keepIds: string[],
): void {
  const db = getDb();
  if (keepIds.length === 0) {
    db.prepare(`DELETE FROM tickets WHERE project_key = ? AND imported_from = ?`).run(
      projectKey,
      importedFrom,
    );
  } else {
    ...
    db.prepare(
      `DELETE FROM tickets WHERE project_key = ? AND imported_from = ? AND ticket_id NOT IN (${placeholders})`,
    ).run(projectKey, importedFrom, ...keepIds);
  }

  // Sweep dependent rows orphaned by the ticket deletion above.
  for (const t of ["ticket_events", "ticket_relations", "blocker_evidence"]) {
    db.prepare(
      `DELETE FROM ${t} WHERE project_key = ?
         AND ticket_id NOT IN (SELECT ticket_id FROM tickets WHERE project_key = ?)`,
    ).run(projectKey, projectKey);
  }
}

// Reconcile relations to EXACTLY the desired set: removes relations belonging to
// removed tickets AND `related:` entries dropped from a surviving ticket.
export function reconcileRelations(
  projectKey: string,
  keepIds: string[],
  desired: TicketRelation[],
): void {
  if (keepIds.length === 0) return;
  const db = getDb();
  const keep = new Set(keepIds);
  const existing = db
    .prepare(`SELECT ticket_id, related_id, rel_type FROM ticket_relations WHERE project_key = ?`)
    .all(projectKey) as { ticket_id: string; related_id: string; rel_type: string }[];
  const want = new Set(desired.map((d) => `${d.ticketId} ${d.relatedId} ${d.relType}`));
  const del = db.prepare(
    `DELETE FROM ticket_relations WHERE project_key = ? AND ticket_id = ? AND related_id = ? AND rel_type = ?`,
  );
  for (const r of existing) {
    if (!keep.has(r.ticket_id)) continue; // not this source's ticket
    if (!want.has(`${r.ticket_id} ${r.related_id} ${r.rel_type}`)) {
      del.run(projectKey, r.ticket_id, r.related_id, r.rel_type);
    }
  }
}

// A ticket that was legacy-blocked and is no longer blocked in Markdown must also
// lose its import-managed blocker_evidence. Source-scoped so future Slice D
// enrichment rows are untouched. blockedIds = ticket_ids currently status:blocked.
export function reconcileImportedBlockerEvidence(
  projectKey: string,
  keepIds: string[],
  blockedIds: string[],
  source: string,
  ...
  const del = db.prepare(
    `DELETE FROM blocker_evidence WHERE project_key = ? AND source = ? AND ticket_id = ?`,
  );
  for (const id of keepIds) {
    if (!blocked.has(id)) del.run(projectKey, source, id);
  }
}
