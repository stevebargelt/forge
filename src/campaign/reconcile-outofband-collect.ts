...
import { checkClosedCommitReachableOnBase } from "./reconcile-collect.js";
...
export function collectOutOfBandEvidence(projectDir: string, item: CampaignItem): OutOfBandEvidenceInput {
  ...
  if (ticketClosedCommit) {
    if (commitTouchesOnlyNonCodePaths(projectDir, ticketClosedCommit)) {
      ...
      try {
        // Bound to the SAME ticketClosedCommit asserted above — an operator cannot
        // plant a verification row under a different sha and have it count.
        ...
        const allExitZero = recorded && rows.every((r) => r.exitCode === 0);
        if (recorded && allExitZero) {
