...
const RESTORE_CONTRACT = [
  ...
  "     rename is the ONLY step that touches the live path — any failure during",
  "     validation or staging leaves forge.db byte-for-byte untouched, and a",
  "     pre-restore safety backup of the CURRENT store is taken first.",
  ...
  "     can take exclusive access to the live store (no other connection holds it).",
  "     This is a BACKSTOP, not a hard fence — an uncooperative writer that predates",
  "     backup.lock is not machine-fenced; your quiescence (point 2) is what makes",
  "     the window safe, exactly the honest limit `forge store converge` documents.",
