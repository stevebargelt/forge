  ...
  notify
    ...
    .action(async (opts: { run: string; kind: string; title: string; body?: string; dedupeKey?: string; json?: boolean }) => {
      try {
        ...
        if (opts.json) {
          console.log(JSON.stringify({ recorded: true, kind: opts.kind, dispatched: res.dispatched, reason: res.decision.reason, importance: res.importance, ...(res.dispatchError ? { dispatchError: res.dispatchError } : {}), ...(res.docsImpactWarning ? { docsImpactWarning: res.docsImpactWarning } : {}) }, null, 2));
          return;
        }
        ...
        if (res.decision.send && !res.dispatched) {
          if (res.dispatchError) {
            console.log(`  (delivery failed: ${res.dispatchError})`);
          } else {
