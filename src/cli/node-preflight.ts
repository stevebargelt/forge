...
import { REQUIRED_ABI, checkAbi } from "./node-abi.js";

// FG-555: the bounded ABI-equality assertion (FG-570) now lives in the
// side-effect-free node-abi.ts so other modules (the launch-toolchain refusal)
// can REUSE it without triggering this module's import-time applyNodePreflight().
// Re-exported here so existing consumers (node-preflight.test.ts,
// node-preflight.integration.test.ts) keep importing from "./node-preflight.js".
export { REQUIRED_ABI, checkAbi };
