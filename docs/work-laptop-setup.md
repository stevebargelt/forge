...
`forge upgrade` refreshes `~/.forge/` — on this first run `~/.forge/` is empty, so it installs everything (agent seeds, constraints, workflow and runtime YAML, the RACI) and compiles the derived routing policy. This first run also completes the FG-776 one-time host-edit backup (nothing to back up on an empty `~/.forge/`) and writes its completion latch, so on this and every later machine every seed category — including agents, constraints, and `forge-raci.md` since FG-777 — is **forge-owned and ALWAYS upgraded**: `forge upgrade` (FORCE) overwrites a locally edited copy of any of them on the next run, the same way it refreshes a runtime. Don't hand-edit `~/.forge/agents/<role>/CLAUDE.md`, `~/.forge/constraints/`, or `~/.forge/forge-raci.md` expecting it to survive — put customization in `<project>/.forge` instead (an agent addendum, an additive constraint, or a full RACI override), which upgrade never touches. The review protocol the lifecycle's roles are dispatched under is forge's rather than yours, and it is not in your agent seed at all: upgrade publishes it inside the seed generation, and your files are never written for that part (FG-654) — see `docs/how-to-upgrade.md`. Add `--rebuild-image` when the agent image hasn't been built on this machine yet.
...
>
> Seed installation itself is *not* dev-only: `forge upgrade` from the release installs the **release's own** bundled seeds and works on a host with no checkout at all. That's the repair path when `~/.forge/`'s forge-owned seeds — agents, constraints, and RACI included, since FG-777 — are broken or drifted, and (once the one-time host-edit backup has run) it repairs those the same as any other seed rather than merely retaining them — see `docs/how-to-upgrade.md`.
...
Model-policy authoring flags (`--default-profile`, `--reasoning`, `--pin
<role>=<profile>`, …) are documented in `docs/how-to-model-policy.md`.
