...
- **Workflow loader** (`src/spine/workflows.ts`) — `loadWorkflow(name)` looks up the new name first; if not found, checks an alias map (`feature-design-needed` → `feature`, `feature-design-provided` → `feature-ui-design-provided`, `design-revise` → `ui-design-revise`). Existing run rows resolve through the alias map without breaking.
