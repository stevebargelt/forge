...
| **F31** | Incompatible ABI | **Refused before native load** | A release manifest naming an ABI this interpreter does not have → **named refusal**. Pass condition **IS a clean refusal**, not a successful run and not `ERR_DLOPEN_FAILED`. (Originally staged with a real Node 26/ABI 147; FG-647 replaced that with the manifest-manufactured mismatch, which proves the same ordering with no second interpreter and no skip.) | Asserting "the control plane runs" — that would **reject a correct F31.** |
