...
> **SUPERSEDED / INCOMPLETE (row: abandoned/cancel races) — the PRD (S3 / INV-2) governs.** The no-resurrection
> property is a **STORE-layer** guard (`completeRun`'s `AND status='active'` at `store/runs.ts:147` +
> `updateRunStatus`'s FG-484 `abandoned→complete` refusal at `:174-179`), **not** `completeRun`'s single caller;
> a single-caller/grep assertion does **not** establish it. Completion is **also** written outside the finalize
> boundary by `design.ts`/`claude.ts` via `updateRunStatus`, so run-finalize is **not** the sole completion
> writer in the tree. The AC holds, but for the store-layer reason, not the one cited here.
...
| **C5** | **Run state + run completion** through the evaluator: `runNext.ts:298-303`'s `topLevelByPhase` / `anyFailed` superseded-primary logic is a **fourth** lineage heuristic — it must become evaluator-derived. `gate.ts:457-461` (advance/reject) and `run-finalize.ts` keep the single-writer boundary. | Today `runNext.ts:299` counts an ad-hoc invoke failed row in `anyFailed`; assert a failed `forge invoke` row no longer marks a workflow run's completion as `anyFailed`. **SUPERSEDED BY PRD → the run-state surface does not exist: NORMATIVE-UNMET (N-2), not a baseline red. Its ad-hoc-`anyFailed` defect is PRD A-6 — INFERENCE, NOT yet observed (a pending precondition, not a met claim). The story cell's "single-writer boundary" is corrected by S3/INV-2: run-finalize is NOT the sole completion writer; the no-resurrection guard is store-layer. `docs/prds/workflow-lifecycle-semantics.md` §7.2 / INV-2 govern.** | *"runNext uses the evaluator for run-completion"*, *"gate uses it after advance/reject/request-changes"* |
| **C6** | **Terminal blockers**: `executor.ts:494` and `:2183` consume the evaluator's failed-primary set instead of `parentId === undefined`. SHARED-wins stays in `policy.ts`. | A failed ad-hoc invoke row attached to a campaign workflow run is today classified as a workflow terminal blocker (`executor.ts:494` → `classifyFailureKind`). Assert it is excluded. Plus the mixed local/shared case (`policy.ts:139-149`) as a regression pin. **SUPERSEDED BY PRD → the terminal-blocker surface does not exist: NORMATIVE-UNMET (N-4), not a baseline red. The ad-hoc-terminal-blocker defect (`executor.ts:494`) is PRD A-6 — INFERENCE, NOT yet observed; its red is a pending precondition. `docs/prds/workflow-lifecycle-semantics.md` §7.2 governs.** | *"campaign resume uses evaluator-derived terminal blocker state"*, *"mixed failed-primary runs classify conservatively"* |
...
- **OQ-2 — Is `retry.ts:263`'s `red-` prefix fallback (mount mode when no receipt exists) in scope?** Default:
  **no.** It asks a security question, not a lineage one.
  > **SUPERSEDED / corrected — the PRD (OQ-2 / D-6) governs:** this fallback fails **OPEN**, not "closed" — a
  > non-prefixed red gets a **writable** mount. That is plausibly a **security finding for another cluster** (the
  > PRD does not scope it to Lane B specifically). **Flagged, not fixed — it still needs an owner.**
