...
| `merge_conflict` failure kind | **reads** — in the INSPECTION-RETAIN set → **retain** the worktree (D9b, `failure-kind.ts:125-143`); adds **no** kind (A §4) | — | **reads** — `merge_conflict` → **SHARED** blocker → holds the whole campaign (S5, `policy.ts:144`; C §5.2(b)) | Same kind, two disjoint consumers → COEXIST. **Neither may broaden `merge_conflict`'s membership unilaterally** — C §5.2(b) warns broadening it broadens campaign holds and says that decision "should be in the worktree ticket, not discovered in a campaign." |
...
**Reading the table (by class):** rows 1–7 are **OWNED-BY** a single owner (a cluster's PRD for rows 1–6, the
pre-existing store invariant for row 7 — the owner decides, the others conform). Rows 8–10 are **COORDINATED**
(a shared region/constant no single cluster owns the merge of — row 8 & 10 are sub-case (a) SAME-REGION, row 9 is
sub-case (b) DISJOINT-REGION/JOINT-INVARIANT; the map sequences them — §2, §4). Row 11 is **OWNED-BY-C** (AC-2 rejected-and-corrected). Rows 12–13 are the two
