...
- **OQ-INT-3 (OWNED-BY-C, proposed child C-FG477-S7) — dashboard + campaign-runner consumption of C's
  ...
  **proposed bounded child C-FG477-S7** (added to the decomposition) — **one child, two consumers.** **The
  ...
  and **re-derive NONE of it.** The operator-reason API is itself PLANNED NEW (S6 output, UNBUILT — no such
  surface in `src/` today), so both consumptions are NEW integrations of a NEW API. **FILES/SCHEMAS (VERIFIED
  existing consumers; NEW route/serialization owner C-FG477-S7):** dashboard = `dashboard/src/server.ts:91-96`
  ...
  `:676` detail render — the operator-reason **route + serialization schema are NEW**. campaign =
