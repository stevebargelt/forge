...
| **16** | **Lane A OQ-4 — should reds get a git-read capability** (Bash, or a narrower git-read tool, now that the `:ro` mount makes reads safe) | A (**punts** it to the Review-trust lane), **Cluster B (Review Execution Trust) OWNS it here** | **OWNED-BY-B.** **(1) OWNER:** Cluster B (Review Execution Trust) — A punted the capability decision to the Review-trust lane. **(2) DECISION AUTHORITY:** B decides whether reds gain a git-read capability (Bash or a narrower git-read tool) now that the `:ro` mount makes reads safe. **(3) SCOPE:** the red-agent git-read capability decision only. **(4) FILES/SCHEMAS:** VERIFIED existing seed files, each `{"tools": ["read"], "notes": "… read-only … enforced with -v <project>:/project:ro"}`: `seeds/agents/red-wide/settings.json`, `seeds/agents/red-narrow/settings.json`, `seeds/agents/red-security/settings.json`, `seeds/agents/red-backend/settings.json`, `seeds/agents/red-frontend/settings.json`. **Loader/propagation (VERIFIED, and it is NOT a tools loader):** no code in `src/` reads a red seed's `settings.json` `tools` field — grepping `settings.json`/`"tools"` across `src/` hits only `.vscode`/`.claude` handling (`project-meta.ts`, `init.ts`) and `role-capabilities.ts` (an unrelated in-TS narrative-role Set). The read-only capability is enforced **solely by the PROJECT_MODE-keyed `/project` bind mount at `src/v2/spawn.ts:255-257`** (mode substituted from `SpawnContext.PROJECT_MODE`, field `spawn.ts:40`), which is set `"ro"` for reds at `src/v2/runNext.ts:1338` (`projectMode: "ro"`, workflow) and `src/v2/invoke.ts:485`/`:525` (`args.readOnlyProject ? "ro" : "rw"`, invoke), plus the receipt/`red-` fallback at `retry.ts:263`. (`spawn.ts:462` is a different mount — the dependency-provisioner container's unconditional `:ro` in `buildProvisionerDockerArgs`, not the agent `/project` mount.) So the seed `tools: ["read"]` is documentary; adding a git-read capability is NOT a settings.json edit that any loader honors — it requires a change to what the `:ro` mount permits (i.e. the mount is the enforcement surface, owner A-FG559's D1/D2). **(5) ACCEPTANCE:** a recorded B decision (keep read-only, or add git-read with rationale + the reviewable capability change — which changes the mount contract, not the decorative seed field). **(6) DEPENDENCIES:** A-FG559 (the `:ro` mount precondition that makes reads safe). **The decision B must make (open UNDER B):** keep `read`-only vs add git-read. | OWNED-BY-B — owner assigned; PRD-a OQ-4 punt VERIFIED, formally accepted by B here (PRD-b's own OQ-4 = unrelated FG-553 revalidation trigger) |
  ...
  **three** OQs explicitly to "the Review-trust lane": **OQ-3** (the `test:unit` integration-gate silent no-op,
  ...
  lane PRD-a named — as the single accountable decider of each:
    ...
    - **OQ-INT-5 (row 16):** B decides whether reds gain a git-read capability now the `:ro` mount makes reads
      safe. FILES: seed files `seeds/agents/red-{wide,narrow,security,backend,frontend}/settings.json` (each
      `{"tools":["read"]}`) — but these are **documentary**: no `src/` code loads the seed `tools` field, so the
      capability is enforced ONLY by the PROJECT_MODE-keyed `/project` bind mount (`spawn.ts:255-257`, mode from
      `SpawnContext.PROJECT_MODE` field `spawn.ts:40`; set `"ro"` for reds at `runNext.ts:1338` /
      `invoke.ts:485`/`:525` / `retry.ts:263` — not `spawn.ts:462`, the provisioner-container mount). A git-read grant changes the mount contract (A-FG559 D1/D2), not the seed file. DEPENDS ON
      A-FG559 (the `:ro` precondition).
