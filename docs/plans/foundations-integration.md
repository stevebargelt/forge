...
- **OQ-INT-3 (OWNED-BY-C, proposed child C-FG477-S7) — dashboard + campaign-runner consumption of C's
  ...
  **proposed bounded child C-FG477-S7** (added to the decomposition) — **one child, two consumers.** **The
  ...
  `:676` detail render — the dashboard transport = a **NEW read route in `dashboard/src/server.ts`** + a **NEW query
  function in `dashboard/src/queries.ts`** + a **NEW render in `dashboard/client/main.js`**, **creation owner
  C-FG477-S7**. campaign =
  `src/campaign/executor.ts:104-105` (its OWN `blockerKind`/`reason`, populated at `:326/:352/:373-374/:560/:621/
  :995/:1370` — NOT the FG-477 operator-reason); the S6-API consumption is a NEW path that must state whether it
  **augments or replaces** that own `reason`. Test surfaces: `dashboard/src/queries-inflight-status.test.ts` (+
  siblings) and the campaign executor tests. **DEPENDENCIES:** C-FG477-S6 (the API/contract must exist first).
