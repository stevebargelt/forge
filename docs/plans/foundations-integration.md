# Cross-plan integration MAP — post-FG-561 foundations program
...
| **7** | **Run completion writes** (no-resurrection) | A, B, C (all must not cross) | **OWNED-BY-store-layer** — a **pre-existing store invariant** (no-resurrection: `completeRun`'s `AND status='active'` `src/store/runs.ts:147`; `updateRunStatus`'s refusal `:174-179`), **not owned by any single cluster's PRD**. **All three clusters conform** — none may add a **third** completion-writing path. | VERIFIED — C INV-2/S3 |
...
| **12** | `retry.ts:263` **fail-open mount-mode fallback** (a non-prefixed red gets a **writable** mount) | C (flags, does not fix), A (security boundary D2/D10) | **OWNED-BY-A.** **(1) OWNER:** Cluster A (workspace-isolation security boundary, D2/D10). **(2) DECISION AUTHORITY:** A decides whether D10b's mount-mode-keyed pointer-freeze (rw ⇒ freeze, independent of agent class) already covers a fail-open red, OR a mitigation is required. **(3) SCOPE:** audit `retry.ts:263`'s mount-mode fallback against D10b; decide covered-vs-mitigation. **(4) FILES/SCHEMAS:** VERIFIED existing — `src/v2/retry.ts:263` inside `planAdHocRedispatch` (fn opens `retry.ts:232`): the receipt-less mount-mode fallback `const readOnlyProject = receipt ? receipt.mountMode === "ro" : task.agentRole.startsWith("red-")` — a red with no `mountMode:"ro"` receipt falls back to the role-name discriminator and, if mis-prefixed, gets a **writable** mount. PLANNED NEW (owner **A-FG559**, does NOT exist in `src/` today): the D10b worktree-local `.git` pointer-freeze — `docs/plans/foundations-decomposition.md:53` (D10/D10a/D10b bullet) and `:92-93` ("**D10b (pointer `:ro`) is the load-bearing closure**"). The mount-mode decision A must audit `:263` against is the same PROJECT_MODE-keyed `/project` bind mount at `src/v2/spawn.ts:255-257` (mode substituted from `SpawnContext.PROJECT_MODE`, field `spawn.ts:40`), set `"ro"` for reds at `src/v2/runNext.ts:1338` (workflow) and `src/v2/invoke.ts:485`/`:525` (invoke). (Not `spawn.ts:462`, which is the dependency-provisioner container's unconditional `:ro` mount in `buildProvisionerDockerArgs`, not the PROJECT_MODE-keyed agent mount.) **(5) ACCEPTANCE:** a recorded decision — either "D10b covers it (rw ⇒ freeze regardless of class), evidence cited" **or** a named mitigation (e.g. `retry.ts` must not grant rw to a red). **(6) DEPENDENCIES:** A-FG559 (mount/D10 work); if a `retry.ts` fix is chosen, COORDINATE with **C** (C-FG527-a owns `retry.ts` and its scope must-not-touch `:263`). **The decision A must make (open UNDER A):** covered-by-D10b vs mitigation-required — the architectural outcome stays open, but the row is OWNED. | OWNED-BY-A — owner assigned; §3.2 OQ-INT-1 (C D-6/OQ-2 VERIFIED; A-boundary fit INFERENCE) |
| **13** | **Held-child worktree reclaim at RUN-END** (run ends with the child still `awaiting_gate`) | B (N-9(b) requires reclaim), A (reaper is terminal-only) | **OWNED-BY-B (proposed child B-FG524b).** **(1) OWNER:** proposed bounded child **B-FG524b** (Cluster B) — added to the decomposition. **(2) DECISION AUTHORITY:** B decides the run-end reclaim mechanism — **(i)** run-end/abandon **terminalizes** held children (then A-FG356's reaper collects them), or **(ii)** B owns a direct run-end reclaim path. **STATE-TRUTH CONSTRAINT on (i):** an unresolved `awaiting_gate` hold **cannot become `complete`** (that asserts a validation pass that never happened). The truthful cancellation shape is **TaskStatus=`failed` backed by durable task-level evidence** — `failure_kind=cancelled` (a **FailureKind/event, NOT a TaskStatus**) plus `task.cancelled`; **RunStatus=`abandoned` alone does not terminalize or prove the held child's transition.** Option (i) is admissible **only if** a real cancellation/abandonment path **causally transitions the held child from `awaiting_gate` to TaskStatus=`failed` and records that task-level evidence** — never a status flip synthesized just to match A-FG356's `{complete, failed}` predicate. If no such causal, evidence-backed transition exists, (i) is inadmissible and the mechanism must be (ii). Selected/recorded/reviewed in the decision-closure gate before implementation. **(3) SCOPE:** the run-end reclaim of a still-held fanout child's worktree ONLY (retention-while-held stays in B-FG524). **(4) FILES/SCHEMAS:** all VERIFIED existing (no new file, no `tasks` migration): reaper hook = the tail of `src/v2/reconcile.ts:393` `reconcileRun` (A-FG356's additive terminal pass; `finalizeOrphanedPrimaries` at `reconcile.ts:1361`); run-end/abandon finalize = `src/v2/run-finalize.ts:31` `finalizeRunIfSettled` (called on the settled-no-dispatch run-end at `runNext.ts:220`, imported `runNext.ts:38`; also imported into reconcile at `reconcile.ts:27`); worktree/branch reclaim = `src/v2/worktree-lifecycle.ts:179` `removeWorktreeIfSafe` (already invoked from reconcile at `reconcile.ts:570/682/1003`). **(5) ACCEPTANCE:** a run ending with a held child does NOT leak its worktree/branch; a test drives run-end-while-held → reclaimed. **(6) DEPENDENCIES:** A-FG356 (reaper terminal-only), B-FG524 (held-child retention), the `awaiting_gate ∈ non-terminal` constant. **The decision B must make (open UNDER B):** terminalize-then-reap (i) vs direct-reclaim (ii). | OWNED-BY-B — owner assigned (B-FG524b); §3.2 OQ-INT-2 (B N-9(b) VERIFIED; the gap INFERENCE) |
| **14** | **Dashboard + campaign-runner consumption of C's operator-reason API** (C-FG477-S6 / N-5 names `dashboard` and `campaign` as consumers over a **serialization/API transport**) | C (owns the operator-reason contract; S6 **BOUNDS this OUT**), the dashboard (`dashboard/src/{server,queries}.ts` + `client/main.js`), the campaign executor (`src/campaign/executor.ts`) | **OWNED-BY-C (proposed child C-FG477-S7).** **(1) OWNER:** proposed bounded child **C-FG477-S7** (Cluster C) — added to the decomposition. **(2) DECISION AUTHORITY:** C decides/audits how the dashboard (`dashboard/src/queries.ts`+`server.ts`+`client/main.js`) and the campaign executor (`src/campaign/executor.ts`) consume the serialized operator-reason contract. **(3) SCOPE:** audit BOTH consumer transports (dashboard + campaign) of the operator-reason API — one bounded child, two consumers. **(4) FILES/SCHEMAS (each NEW surface is an explicitly-named planned file/schema with exactly ONE creation owner):** the **NEW SCHEMA** is the S6 operator-reason API contract (a single explanation type / serialization) **in `src/v2/lifecycle-evaluator.ts`, creation owner C-FG477-S6** — the type does NOT exist in that module today; per N-5 it is "an API contract to `show`/`report`/dashboard/campaign … one source, internal union NOT shipped to the client." So **both** consumptions are NEW integrations of a NEW S6-owned schema. **Dashboard consumer (EXISTING subsystem):** the surface that renders run/task state today and would gain the operator-reason read is `dashboard/src/server.ts` route `/api/in-flight` (`server.ts:91-96`, calls `inFlight`) and `/api/task/:id` (`server.ts:208-211`, calls `taskDetail`); query layer `dashboard/src/queries.ts` `inFlight` (`:158`, `SELECT … WHERE t.status IN ('running','awaiting_gate','awaiting_red','blocked_by_red','awaiting_recovery')`) and `taskDetail` (`:320`); client render `dashboard/client/main.js` `InFlightSection` (`:439`) + the status-badge render (`:529-530`) and task-detail render (`:676`). The dashboard transport is three concretely-named additions that read the S6 schema, **creation owner C-FG477-S7**: a **NEW HTTP route `GET /api/operator-reason/:runId`** (proposed name — implementer may rename) added to `dashboard/src/server.ts` (alongside `/api/in-flight` `:91-96` / `/api/task/:id` `:208-211`), a **NEW query function `operatorReason(runId)`** (proposed name — implementer may rename) added to `dashboard/src/queries.ts` (alongside `inFlight` `:158` / `taskDetail` `:320`), and a **NEW client render `OperatorReasonBadge`** (proposed name — implementer may rename) added to `dashboard/client/main.js` (alongside `InFlightSection` `:439` / status badge `:529-530` / detail render `:676`). **Campaign consumer (EXISTING subsystem):** `src/campaign/executor.ts` has its OWN `blockerKind`+`reason` (type at `:104-105`; populated/read at `:326`, `:352`, `:373-374`, `:560`, `:621`, `:995`, `:1370`) — that is the campaign's own hold reason, **NOT** the FG-477 operator-reason. Consuming the S6 operator-reason API is a **NEW path, owner C-FG477-S7** — proposed insertion point: the executor's **hold-reason computation site** (the `:104-105` type, populated at `:326`/`:352`/`:373-374`/`:560`/`:621`/`:995`/`:1370`), where the S6 read (proposed — the call that **replaces or augments the executor's own `reason`**; implementer may rename) is sited; **the audit decides augment-vs-replace.** **Propagation chain:** lifecycle-evaluator → S6 operator-reason API (NEW) → serialization (NEW) → dashboard route/query (`server.ts` → `queries.ts`) → client render (`main.js`); and → `src/campaign/executor.ts` consumption. **Test surfaces:** dashboard (existing) `dashboard/src/queries-inflight-status.test.ts` (+ siblings `queries-reconcile.test.ts`, `routes-verification.integration.test.ts`); campaign (existing) `src/campaign/executor.integration.test.ts`, `src/campaign/fg521-show-report-agreement.integration.test.ts`, `src/campaign/report.integration.test.ts`; plus **NEW per-transport contract tests (creation owner C-FG477-S7)** — a dashboard contract test (proposed, alongside `queries-inflight-status.test.ts`; implementer may rename) and a campaign contract test (proposed, alongside `executor.integration.test.ts`; implementer may rename), each asserting the consumer reads the operator-reason contract and re-derives no "why" of its own. **(5) ACCEPTANCE:** each named consumer reads the operator-reason contract and RE-DERIVES NONE of it; hollow = a consumer computing its own "why". **(6) DEPENDENCIES:** C-FG477-S6 (the operator-reason API/contract must exist first). **The decision C must make (open UNDER C):** confirm both transports consume the contract without re-derivation — S6 previously bounded this out with no named owner; C-FG477-S7 is now that owner. | OWNED-BY-C — owner assigned (C-FG477-S7); C-FG477-S6 BOUND-OUT / C OQ-4 (VERIFIED via decomposition C-FG477-S6) |
| **15** | **Lane A OQ-3 — the `test:unit` integration-gate silent no-op** (`integration-gate.ts:63-65` returns `{ok:true, "integration gate skipped"}` for any project with no `test:unit` script) | A (**punts** it to the Review-trust lane), **Cluster B (Review Execution Trust) OWNS it here** | **OWNED-BY-B.** **(1) OWNER:** Cluster B (Review Execution Trust) — A explicitly punted the publication-gate trust decision to "the Review-trust lane"; formally assigned to B here. **(2) DECISION AUTHORITY:** B decides the no-`test:unit` gate policy — keep skipping / record a durable "gate not enforced" marker (PRD-a's suggested default) / refuse to publish without a gate. **(3) SCOPE:** the integration-gate no-op policy for projects lacking a `test:unit` script. **(4) FILES/SCHEMAS:** VERIFIED existing — `src/v2/integration-gate.ts:62` `runIntegrationGate`, whose skip is `:63-64` `if (!hasTestUnitScript(dir)) return { ok: true, output: "no test:unit script in package.json — integration gate skipped" }`; predicate `hasTestUnitScript` at `:39` (reads `pkg.scripts["test:unit"]`). A durable-marker default would add to this same function; no new file required. **(5) ACCEPTANCE:** a recorded B decision; if the marker default is chosen, a durable visible "gate not enforced" marker on the publication attempt. **(6) DEPENDENCIES:** no additional semantic dependency beyond the mandatory program-wide post-FG-561 delta-audit and decision-closure gates. No child or implementing decision may dispatch before those gates pass. **The decision B must make (open UNDER B):** skip / durable-marker / refuse. B may adopt PRD-a's suggested default (durable marker) or override with rationale. | OWNED-BY-B — owner assigned; PRD-a OQ-3 punt VERIFIED, formally accepted by B here (PRD-b's own OQ-3 = unrelated cache-key scheme) |
| **16** | **Lane A OQ-4 — ACCEPT-vs-ENFORCE the already-effective reds git-read** (P7 executed under two genuinely-distinct manifest-verified runtimes — codex-subscription and claude-oauth: a read-only red runs Bash + `git rev-parse`/`git log`; Pi is not tool-capable for reds on this host, so no Pi red execution exists — reds ALREADY POSSESS effective git-read; the `:ro` mount denies filesystem WRITES only, a distinct property) | A (**punts** it to the Review-trust lane), **Cluster B (Review Execution Trust) OWNS it here** | **OWNED-BY-B.** **(1) OWNER:** Cluster B (Review Execution Trust) — A punted the capability decision to the Review-trust lane. **(2) DECISION AUTHORITY:** NOT grant-vs-withhold. Reds **already** run Bash and read git — **VERIFIED FACT, executed** under two genuinely-distinct manifest-verified runtimes, codex-subscription and claude-oauth (`docs/plans/foundations-lane-a-probes/p7-red-runtime-capability.out` SECTION 1); `bash`+`git` ship in the agent image at `/usr/bin/{bash,git}`, so the capability is image-level and holds for any runtime that can dispatch a red (Pi cannot on this host — an INFERENCE corroborated by the two executed runtimes, not a per-runtime execution claim). So B decides **ACCEPT-vs-ENFORCE**: **(a)** ACCEPT and formally support the already-effective git-read; OR **(b)** DEFINE A REAL ENFORCEMENT mechanism that removes command execution across ALL supported runtimes — a runtime/OS-level control that is a **PLANNED NEW mechanism with a creation owner B must name** (prose in CLAUDE.md and the seed `tools` field are NOT enforcement). **(3) SCOPE:** the accept-vs-enforce decision on the reds' already-effective git-read only. **(4) FILES/SCHEMAS:** VERIFIED existing seed files, each `{"tools": ["read"], "notes": "… read-only … enforced with -v <project>:/project:ro"}`: `seeds/agents/red-wide/settings.json`, `seeds/agents/red-narrow/settings.json`, `seeds/agents/red-security/settings.json`, `seeds/agents/red-backend/settings.json`, `seeds/agents/red-frontend/settings.json`. **Loader/propagation (VERIFIED, and it is NOT a tools loader):** no code in `src/` reads a red seed's `settings.json` `tools` field — grepping `settings.json`/`"tools"` across `src/` hits only `.vscode`/`.claude` handling (`project-meta.ts`, `init.ts`) and `role-capabilities.ts` (an unrelated in-TS narrative-role Set). The PROJECT_MODE-keyed `/project` bind mount at `src/v2/spawn.ts:255-257` (mode substituted from `SpawnContext.PROJECT_MODE`, field `spawn.ts:40`), set `"ro"` for reds at `src/v2/runNext.ts:1338` (`projectMode: "ro"`, workflow) and `src/v2/invoke.ts:485`/`:525` (`args.readOnlyProject ? "ro" : "rw"`, invoke), plus the receipt/`red-` fallback at `retry.ts:263`, enforces **filesystem WRITE-DENIAL ONLY** (P7 executed: a red's `touch`/redirect are refused `Read-only file system`, exit 1) — a property **DISTINCT from the git-read capability**. Changing the mount does NOT grant or remove git-read; the seed `tools: ["read"]` field and CLAUDE.md prose are **INTENT, NOT enforcement** (no `src/` loader reads the `tools` field — grepping `settings.json`/`"tools"` across `src/` hits only `.vscode`/`.claude` handling in `project-meta.ts`/`init.ts` and `role-capabilities.ts`; compose reads CLAUDE.md). (`spawn.ts:462` is a different mount — the dependency-provisioner container's unconditional `:ro` in `buildProvisionerDockerArgs`, not the agent `/project` mount.) So the five seed files stand as **documentary intent** and the mount stands as the **WRITE-enforcement surface** (owner A-FG559's D1/D2), neither of which is the git-read enforcement surface. **(5) ACCEPTANCE:** a recorded B **accept-or-enforce** decision, grounded in the **P7 executed evidence** (not a source pattern): either accept-and-formally-support the already-effective git-read, OR a named PLANNED NEW runtime/OS-level enforcement mechanism (with its creation owner) that removes command execution on every supported runtime. **(6) DEPENDENCIES:** FG-559 repairs repository **VISIBILITY** for linked worktrees — **NOT tool availability**; reds can already read git, so FG-559 is neither the precondition that grants git-read nor what makes reads "safe." **The decision B must make (open UNDER B):** ACCEPT the already-effective git-read vs ENFORCE its removal via a real runtime-level mechanism. | OWNED-BY-B — owner assigned; P7 (executed) proves reds already have effective git-read, so the punt is reframed accept-vs-enforce, formally accepted by B here (PRD-b's own OQ-4 = unrelated FG-553 revalidation trigger) |
| **17** | **Lane A OQ-7 — a LEGIT operator-installed common-config driver** (e.g. `git-lfs` smudge/clean, a repo `diff.*.textconv`) **triggered on agent-chosen tracked paths via an agent-written `.gitattributes`** (attacker-influenced input to operator-trusted code) | A (**punts** it, accepts for its own cluster only), **Cluster B (Review Execution Trust) OWNS it here** | **OWNED-BY-B.** **(1) OWNER:** Cluster B (Review Execution Trust) — A punted it ("out of this cluster's scope, Review-trust lane"). **(2) DECISION AUTHORITY:** B decides neutralize-vs-accept for the attribute-TRIGGERED legit-driver exec. **(3) SCOPE:** the neutralize-or-accept decision for attribute-triggered legit-driver exec at host bookkeeping git. **(4) FILES/SCHEMAS:** the D10a "single hardened host-git wrapper" is a **PLANNED NEW path, ONE creation owner A-FG559** — it does NOT exist in `src/` today (`docs/plans/foundations-decomposition.md:53-56` and A-FG559 FILES/SCHEMAS `:63-68`: "a single hardened host-git wrapper prepending fixed-key `-c core.hooksPath=/dev/null -c core.fsmonitor=`"). OQ-INT-6's mitigation (`-c filter.<name>.process=` / `GIT_LFS_SKIP_SMUDGE`) **EXTENDS** that NEW wrapper. **Affected call/propagation boundaries (VERIFIED existing host-git sites the wrapper guards):** `autoCommitSource` (`src/v2/integration-publisher.ts:304`, git exec at `:305/307/308`), `changedWorktreeFiles` (`src/v2/reconcile.ts:229-240`), `mergeChildIntoIntegration` (`src/v2/worktree-lifecycle.ts:410`, git execs at `:420/434/439`). **(5) ACCEPTANCE:** a recorded B decision (accept, or neutralize via the wrapper). **(6) DEPENDENCIES:** A-FG559 / D10a (the host-git wrapper the mitigation would extend) — a NOTED cross-cluster coordination. **The decision B must make (open UNDER B):** accept vs neutralize-via-wrapper. | OWNED-BY-B — owner assigned; PRD-a OQ-7 punt VERIFIED, formally accepted by B here |
...
| **Run completion writes** (no-resurrection) | **store layer** owns the guarantee (`src/store/runs.ts:147`, `:174-179`) | all clusters must **not add a third completion-writing path** (C INV-2) | C INV-2/S3 — a boundary A and B must not cross |
...
- **OQ-INT-2 (OWNED-BY-B, proposed child B-FG524b) — run-end reclaim of a still-held child's worktree.**
  ...
  **The decision B must make (open UNDER B):** either **(i)** run-end/abandon *terminalizes* held children (then
  ...
  run-end arm ONLY — retention-while-held stays in B-FG524. **FILES (all VERIFIED existing):** reaper hook = tail
  of `reconcile.ts:393` `reconcileRun` (`finalizeOrphanedPrimaries` `:1361`); run-end/abandon finalize =
  `run-finalize.ts:31` `finalizeRunIfSettled` (run-end call `runNext.ts:220`); worktree reclaim =
  `worktree-lifecycle.ts:179` `removeWorktreeIfSafe`. **DEPENDENCIES:** A-FG356 (terminal-only
...
- **OQ-INT-3 (OWNED-BY-C, proposed child C-FG477-S7) — dashboard + campaign-runner consumption of C's
  ...
  **proposed bounded child C-FG477-S7** (added to the decomposition) — **one child, two consumers.** **The
  decision/audit C must make (open UNDER C):** confirm both consumers read the serialized operator-reason contract
  and **re-derive NONE of it.** **FILES/SCHEMAS (each NEW surface named, exactly ONE creation owner):** the **NEW
  SCHEMA** = the S6 operator-reason API contract in **`src/v2/lifecycle-evaluator.ts`, creation owner C-FG477-S6**
  (type absent there today; N-5), so both consumptions are NEW integrations of that NEW S6-owned schema. dashboard
  (VERIFIED-existing surfaces) = `dashboard/src/server.ts:91-96`
  (`/api/in-flight` → `inFlight`) + `:208-211` (`/api/task/:id` → `taskDetail`); `dashboard/src/queries.ts:158`
  `inFlight` / `:320` `taskDetail`; `dashboard/client/main.js:439` `InFlightSection`, `:529-530` status badge,
  `:676` detail render — the dashboard transport = a **NEW HTTP route `GET /api/operator-reason/:runId`** (proposed
  name — implementer may rename) in `dashboard/src/server.ts` + a **NEW query function `operatorReason(runId)`**
  (proposed name — implementer may rename) in `dashboard/src/queries.ts` + a **NEW client render `OperatorReasonBadge`**
  (proposed name — implementer may rename) in `dashboard/client/main.js`, **creation owner C-FG477-S7**. campaign =
  `src/campaign/executor.ts:104-105` (its OWN `blockerKind`/`reason`, populated at `:326/:352/:373-374/:560/:621/
  :995/:1370` — NOT the FG-477 operator-reason); the S6-API consumption is a NEW path sited at that **hold-reason
  computation site** (proposed — the call that **replaces or augments the executor's own `reason`**; implementer may
  rename), and **the audit decides augment-vs-replace.** Test surfaces: dashboard (existing)
  `dashboard/src/queries-inflight-status.test.ts` (+ siblings `queries-reconcile.test.ts`,
  `routes-verification.integration.test.ts`); campaign (existing) `src/campaign/executor.integration.test.ts`,
  `src/campaign/fg521-show-report-agreement.integration.test.ts`, `src/campaign/report.integration.test.ts`; plus
  **NEW per-transport contract tests (creation owner C-FG477-S7)** — a dashboard contract test (proposed, alongside
  `queries-inflight-status.test.ts`) and a campaign contract test (proposed, alongside `executor.integration.test.ts`),
  each asserting the consumer reads the operator-reason contract and re-derives no "why" of its own; implementer may
  rename either. **DEPENDENCIES:** C-FG477-S6 (the API/contract must exist first).
  **ACCEPTANCE:** each named consumer reads the contract; hollow = a consumer computing its own "why". C-FG477-S7
  is the single named owner of both NEW consumer transports.
...
- **OQ-INT-4 / OQ-INT-5 / OQ-INT-6 (all OWNED-BY-B) — three Lane A open questions PRD-a punted to the Review-trust
  ...
  **three** OQs explicitly to "the Review-trust lane": **OQ-3** (the `test:unit` integration-gate silent no-op,
  ...
  "Review-trust lane," §4.2; **PRD-a SHA `f1bd97d` reclassified this** — P7 proved reds ALREADY have effective
  ...
  table). PRD-b's own OQ-3/OQ-4 are the **unrelated** cache-key scheme and FG-553 revalidation trigger. **PRD-b now
  textually accepts all three in its §9.4** ("Lane A handoffs ACCEPTED"), so this ownership lives in the normative
  PRD and not only in this map. **The map and PRD-b agree: all three are Cluster B (Review Execution Trust)** — the
  lane PRD-a named — as the single accountable decider of each:
    - **OQ-INT-4 (row 15):** B decides the no-`test:unit` gate policy — keep skipping / durable "gate not enforced"
      marker (PRD-a's default) / refuse to publish. FILES: `integration-gate.ts:62` `runIntegrationGate`, skip at
      `:63-64`; predicate `hasTestUnitScript` `:39`. No blocking dependency.
    - **OQ-INT-5 (row 16):** reframed **accept-vs-enforce**, NOT grant-vs-withhold — reds **already possess
      effective git-read** (**P7 executed** under two genuinely-distinct manifest-verified runtimes —
      codex-subscription and claude-oauth: a read-only red runs Bash + `git rev-parse`/`git log`; Pi is not
      tool-capable for reds on this host, so no Pi red execution exists;
      `docs/plans/foundations-lane-a-probes/p7-red-runtime-capability.out` SECTION 1; `bash`+`git` at `/usr/bin`,
      image-level). B decides **(a)** ACCEPT and formally support it, OR **(b)** name a PLANNED NEW runtime/OS-level
      enforcement mechanism (with a creation owner) that removes command execution on every supported runtime.
      FILES: seed files `seeds/agents/red-{wide,narrow,security,backend,frontend}/settings.json` (each
      `{"tools":["read"]}`) + CLAUDE.md prose are **documentary INTENT, not enforcement** — no `src/` code loads
      the seed `tools` field. The PROJECT_MODE-keyed `/project` bind mount (`spawn.ts:255-257`, mode from
      `SpawnContext.PROJECT_MODE` field `spawn.ts:40`; set `"ro"` for reds at `runNext.ts:1338` /
      `invoke.ts:485`/`:525` / `retry.ts:263` — not `spawn.ts:462`, the provisioner-container mount) enforces
      **filesystem WRITE-DENIAL ONLY** (P7: writes refused, `Read-only file system`, exit 1) — it does NOT grant
      or remove git-read. FG-559 fixes worktree repository **VISIBILITY, NOT tool availability**, so it is not the
      precondition that grants reads.
    - **OQ-INT-6 (row 17):** B decides neutralize-vs-accept for the attribute-triggered legit-driver exec. FILES:
      the D10a host-git wrapper is PLANNED NEW (owner A-FG559, `decomposition:53-56`/`:63-68`); a mitigation adds
  ...
  Each is a recorded B decision (PRD-a's suggested defaults: OQ-3 → durable marker; OQ-4 (post-`f1bd97d`) →
