...
- **OQ-INT-4 / OQ-INT-5 / OQ-INT-6 (all OWNED-BY-B) — three Lane A open questions PRD-a punted to the Review-trust
  ...
  **three** OQs explicitly to "the Review-trust lane": **OQ-3** (the `test:unit` integration-gate silent no-op,
  ...
  "Review-trust lane," §4.2; **PRD-a SHA `c1a77e3` reclassified this** — P7 proved reds ALREADY have effective
  ...
  lane PRD-a named — as the single accountable decider of each:
    ...
    - **OQ-INT-5 (row 16):** reframed **accept-vs-enforce**, NOT grant-vs-withhold — reds **already possess
      effective git-read** (**P7 executed**: a read-only red runs Bash + `git rev-parse`/`git log` under codex,
      claude, AND pi; `docs/plans/foundations-lane-a-probes/p7-red-runtime-capability.out`; `bash`+`git` at
      `/usr/bin`). B decides **(a)** ACCEPT and formally support it, OR **(b)** name a PLANNED NEW runtime/OS-level
  ...
  Each is a recorded B decision (PRD-a's suggested defaults: OQ-3 → durable marker; OQ-4 (post-`c1a77e3`) →
