...
| F31 | incompatible ABI refused before native load | `cli/node-abi.ts:checkAbi`; `node-preflight.ts` | `node-preflight.integration.test.ts` (whole file); `launch-provenance-cli.integration.test.ts:303` | asserts `/refusing to run/` + `doesNotMatch(OPAQUE)` | none — **FG-647:** the mismatch is manufactured in the MANIFEST and every arm runs under the one `.nvmrc` interpreter, so the coverage is deterministic and never skips. The real-second-interpreter arm and the CI Node 26 provisioning it needed are deleted |
