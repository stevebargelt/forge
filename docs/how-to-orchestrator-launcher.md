...
| --- | --- | --- |
| **Instruction source** | Forge's orchestrator policy reaches the session through **one** path. When your `CLAUDE.md` carries a forge block that is exactly what this forge renders for the project's `ai_attribution` (CRLF line endings aside), that block is the delivery and the launch appends nothing; the receipt evidence says so. Any other block — stale, hand-edited, foreign, or markers quoted in a code fence — never suppresses the carrier: the rendered policy is appended and the receipt records the drift as an `instruction-source` limitation recommending `forge upgrade`. A missing, plain, or torn `CLAUDE.md` gets the policy appended instead via `--append-system-prompt-file`, rendered for the project's `ai_attribution` and sliced to the marker region (no unresolved `forge:if` markers, no "Stack + project context" placeholder tail). Your `CLAUDE.md` is never spliced. | **Evidence-gated.** A Forge-owned carrier is bound per launch via a Codex config override, **substituting** (not appending to) the base instruction surface — and only after a positive pre-spawn probe proves the installed `codex` build honors it (AC8). Constructing the flag is not evidence it worked; see `adapter-not-ready` below. |
...
**The bound generation is not always the running one.** Claude Code renders its carrier (when one is needed) from the running tree's own
`seeds/orchestrator-template.md` at launch, so its bound generation is always the running assets. Codex binds its
instruction carrier out of the **published** seed generation instead — one pinned carrier per `ai_attribution` mode
(`codex/orchestrator-instructions.md` for `suppress`, `codex/orchestrator-instructions.allow.md` for `allow`), the
adapter binding the one for the project's mode. A generation published before this change has no `allow` variant, so
an `allow` project records an `instruction-source` limitation until `forge upgrade` republishes it. Because that generation is published, a failed or deferred `forge upgrade`
can leave it behind the forge that is actually executing — comparing the project against the running assets in that
