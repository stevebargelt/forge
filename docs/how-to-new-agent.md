...
- A role **the review lifecycle dispatches** — the five `red-*` lenses plus `engineer`, `documentation-maintainer`, `review-rechecker`, `shipping-reviewer` — gets a second, Forge-owned half you do not write: the review protocol its output is judged against, which lives in the published seed generation as `agent-protocols/<role>.md` and is composed ahead of your `CLAUDE.md` at dispatch. Your file stays wholly yours — edit it freely — but dispatch refuses that role by name when the protocol is missing from the generation, torn, or behind the running release (`forge upgrade` is the remedy), and also when your seed still carries an embedded copy of the protocol from before FG-654 (delete that section by hand). A role you add here is not covered, so nothing refuses on it. See [how-to-upgrade.md → Agent seeds and the Forge-owned protocol](how-to-upgrade.md#agent-seeds-and-the-forge-owned-protocol).
