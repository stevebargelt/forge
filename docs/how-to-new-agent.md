...
- A role **the review lifecycle dispatches** — the five `red-*` lenses plus `engineer`, `documentation-maintainer`, `review-rechecker`, `shipping-reviewer` — is not wholly yours to hand-edit the way a new role like `security-reviewer` is. Its `CLAUDE.md` carries a marker-fenced Forge-owned protocol region that `forge upgrade` publishes and that dispatch refuses when stale; edit outside the fence only. A role you add here is not covered, so nothing refuses on it. See [how-to-upgrade.md → Agent seeds: yours, except the protocol region](how-to-upgrade.md#agent-seeds-yours-except-the-protocol-region).
