...
- A role **the review lifecycle dispatches** — the five `red-*` lenses plus `engineer`, `documentation-maintainer`, `review-rechecker`, `shipping-reviewer` — gets a second, Forge-owned half you do not write: the review protocol its output is judged against, which lives in the published seed generation as `agent-protocols/<role>.md` and is composed ahead of your `CLAUDE.md` at dispatch. Dispatch refuses that role by name when the protocol is missing from the generation, torn, or behind the running release (`forge upgrade` is the remedy), and also when the seed still carries an embedded copy of the protocol from before FG-654 (since FG-777 the next `forge upgrade` clears that too, as part of the same always-upgrade; you can still delete the section by hand instead). A role **you add here yourself** (like `security-reviewer` above) has no shipped `seeds/agents/<role>/` counterpart, so `install-seeds.sh` never visits it at all — it stays wholly yours, edit it freely, and it's not covered by the review protocol either, so nothing refuses on it. A role **forge itself ships** (any of the nine above) is different since FG-777: `~/.forge/agents/<role>/CLAUDE.md` is forge-owned and ALWAYS upgraded — `forge upgrade` (FORCE) overwrites a locally edited copy with the release's, gated only on a one-time host-edit backup — so customize it via a project addendum at `<project>/.forge/agents/<role>/CLAUDE.md` instead, appended after the refreshed base rather than replacing it. See [how-to-upgrade.md → Agent seeds and the Forge-owned protocol](how-to-upgrade.md#agent-seeds-and-the-forge-owned-protocol).
- Constraint files at `~/.forge/constraints/` filter into the agent's prompt by `roles:`, `workflows:`, and `phases:` frontmatter. To make a new constraint apply to your role, list `security-reviewer` in `roles:`. Add `tags: [<tag>]` to scope a constraint to runs created with `forge new --tag <tag>` — useful for project-specific constraints that should not fire on every project's runs. A constraint can also declare `enabled_when: { config: <key>, equals: <value> }` (FG-799) to gate its own inclusion on a project config value — the toggle is the constraint author's own declared condition, not something a project can add after the fact; see [concepts.md → Constraint](concepts.md#constraint).
