...
2. **A run's status is only `active | complete | abandoned` — there is no run-level `failed`.** Success/failure detail lives on individual task rows (`pending|running|awaiting_gate|awaiting_red|complete|failed|blocked_by_red`). A run can legitimately complete while containing failed tasks.
