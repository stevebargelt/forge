...
Additive and nullable, so every pre-FG-689 row — and every review that has not planned yet — reads back `shardPlan === undefined`, which the gate reads as "no plan recorded" rather than as an empty plan against which nothing is owed. Written only by `recordShardPlan`; deliberately absent from `ReviewPatch`, so no ordinary column patch can rewrite what a review is owed. Two events accompany it on the timeline: `review.shard_plan_recorded` (the derivation, the digest, what it supersedes, the expected shard count per lens, the lenses given none) and `review.lens_skipped`.
