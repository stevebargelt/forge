...
- `agentProtocol` — *(optional)* `{ role, sha256, source }` — which generation of the Forge-owned agent protocol region this agent ran under (FG-654). A **sibling** of `controlPlane`, not a field inside it, but written under the same discipline: fixed at dispatch, never recomputed. `sha256` digests the marker-fenced region's content (LF-normalized and trimmed) — the *region*, never the whole file, so an operator's own edits outside the fence cannot change it; `source` is the installed seed the dispatched agent actually read. Present only for a role the review lifecycle dispatches (the five `red-*` lenses plus `engineer`, `documentation-maintainer`, `review-rechecker`, `shipping-reviewer`) — an uncovered role has no region to record. Omitted on pre-FG-654 manifests. It is resolved by the same compose that produced the prompt, not re-derived at manifest-write time, so the prompt and its receipt cannot describe different generations; a retry re-resolves and re-stamps rather than replaying the original. This manifest is AUTHORITATIVE for the fact — the review ledger's copy in `lens_outcomes_json` is an index of it
