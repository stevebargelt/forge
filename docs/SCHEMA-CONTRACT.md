...
#### FG-664: read-only dependency-environment events

The two outcomes of one pre-container decision — the host resolving a read-only dispatch's dependency environment before any agent container starts (see [The read-only dependency-environment gate](concepts.md#the-read-only-dependency-environment-gate-fg-664)). Both set `run_id` and `task_id`, so both surface on the per-task Timeline through a strict `task_id` match, and both are emitted from the same seam on **either** read-only lane (`invoke.ts`'s reviewers/reds and `runNext.ts`'s workflow reds) with the same payload — one fact, one vocabulary, whichever lane reached it. No dashboard consumer reads the success event today; it exists so an auditor asking "which engine did this verification actually run against" reads the timeline rather than a manifest beside one dispatch. The refusal event is read — it is in the agent-runtime attached-exit set, with the consequence noted below.
...
- `dependencyEnvironment` — *(optional)* the ENGINE IDENTITY a Forge-owned probe container attested for this dispatch before its agent container started (FG-664): `{ cacheKey, probeImage, nodeVersion, abi, packages: [{ name, version, artifact?, resolvedPath?, loaded, error? }], staleCacheRepaired? }`. `abi` is `process.versions.modules` inside the probe container — the ABI the mounted bindings had to match to load at all — and `loaded` is the **exit status of a container that did nothing but `dlopen` that one artifact**, not an inference from the file's existence. `staleCacheRepaired` (`{ disprovenRoot, invalidatedMarker, reprovisionedBy: "this_dispatch" | "concurrent_dispatch" }`) is present only when this dispatch found the cache key marked ready and its install root empty in the container, and re-provisioned once. Written on a read-only dispatch that resolved an environment at all — **absent** on every read-write (blue) dispatch, on the not-applicable configurations (non-darwin, `FORGE_NO_NM_SHADOW=1`, no repo-root `package-lock.json`), on a refused dispatch, and on every pre-FG-664 manifest. This manifest is AUTHORITATIVE; the review's `recheck` stage evidence carries the same receipt as an index of it. See [The read-only dependency-environment gate](concepts.md#the-read-only-dependency-environment-gate-fg-664)
- `dispatchRefused` — *(optional)* `{ stage: "dependency_environment", reason, detail }` — WHY this manifest describes a task no container ever ran for (FG-664). Present only on a `forge invoke` dispatch the gate refused pre-container (a refused workflow red returns before its manifest is written, so it has none); `reason` is one of `mountpoints_unavailable | provisioning_failed | probe_failed | probe_unparseable | dependencies_absent | driver_unloadable`. The manifest is written **even though nothing was dispatched**, because `forge retry` recovers a task's mount mode from it — with no manifest it falls back to an `agentRole.startsWith("red-")` guess, which `review-rechecker` does not match, so a refused rechecker would retry READ-WRITE. A manifest carrying it records no `agentProtocol` (no container ran, so no protocol was executed); a manifest without it dispatched
