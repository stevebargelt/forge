...
- **Otherwise `completed_at`, unless the terminal was administrative.** With no such event — none logged, or every one it has predates `started_at` — `completed_at` is the end only if the row is not a `failed` one whose latest `task.failed` payload carries an administrative `failure_kind`: `cancelled`, `gate_rejected`, `orphaned`, `orphaned_work_may_persist`, `orphaned_needs_finalize`, `fanout_wave_orphaned`, `oom_killed`, `pre_container_crash`, `verification_environment_unavailable`. Those rows are written by a process that was *not* supervising the container (`forge cancel`, `forge gate --reject`, the reconcile / ops-repair sweeps), so their `completed_at − started_at` is wall-clock-until-noticed rather than runtime. (`oom_killed` here is reconcile's classification; an attached-exit OOM carries a `container.exited` event and is already resolved by the layer above.) Every other kind is classified by the supervising process at the agent's own exit and counts in full — `idle_timeout` above all, since a killed-while-running agent consumed that time.

When neither layer yields an end the observation is **excluded** rather than counted at an invented duration, so a cancelled task that never logged an exit event is absent from the chart entirely rather than contributing the span until someone noticed it. **Bucket placement is unaffected**: an observation still sits in the bucket its `completed_at` falls in — only the duration moves, so a row whose agent stopped weeks before it was cancelled still plots on its cancellation day, at its real runtime.
