...
#### FG-666: pre-container backlog-authority refusal

- `task.backlog_authority_refused` — a **ticketed** dispatch was refused before its container started, because the backlog authority resolved for the run's project directory could not answer for the ticket the run is anchored to (see [Container ticket authority](concepts.md#container-ticket-authority)). Sets both `run_id` and `task_id`, so it surfaces on the per-task Timeline through a strict `task_id` match. Payload: `{ ticketId, projectDir, mode, stage, failureKind, reason?, detail? }`.

`projectDir` is the run's recorded project directory and is **nullable** — a run with none recorded degrades to an unresolvable authority rather than falling back to the workspace, and records `null` here. `mode` is the resolved authority mode, in practice `"unknown"` or `"db"`: a `markdown` authority is never refused, so this event cannot fire on a normal markdown-mode or non-forge project. `stage` is `"resolve"` (the authority was already degraded when resolved) or `"publish"` (a healthy resolution degraded while publishing the marker or snapshot) — the check runs at both points, and a `"publish"` row means the earlier one passed. `failureKind` duplicates the `failure_kind` landed on the paired `task.failed` row (`backlog_authority_conflict` | `backlog_authority_unresolvable` | `backlog_ticket_unreadable`) so a reader of this event alone gets the retry decision without a second lookup; the two are written from one classification and never disagree. `reason` (`identity-conflict` | `resolve-failed` | `ticket-unreadable`) and `detail` are independently optional, and a `"publish"`-stage row is where they come apart: a publication that degraded an otherwise-healthy `db` resolution — the marker would not write — carries `detail` (the marker error) and **no** `reason`, since the resolution itself had nothing to report. Read `failureKind` for the classification; `reason` is the finer-grained why when there is one.

**No container ever started**, so unlike `container.dependency_provisioning_failed` above this event has no runtime consequence to reconcile: the task never reaches the attached-exit set, and no dashboard consumer reads it today. It exists so the refusal is durable on the run's timeline rather than visible only as a failed task with an error string.
