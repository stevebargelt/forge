...
`run` prints the launch id, the tmux session (`tmux attach -t <session>` to watch it live), and the log path. Poll with `show` — or, better, with forge's own durable state (`forge status <run-id>`, the task rows, the dashboard). Never poll by matching process names: the tmux-owned process is not a child of your shell, and forge's DB is the record that survives a session, a reboot, and a kill. `show` (and `list --json` / `show --json`) also reports the launched workload's own runtime — a `workload:` line for the resolved top-level executable and a `nested:` line for a caller-supplied shell — so you can see *which* toolchain actually ran the command, not just forge's own (FG-555; concepts.md "Durable launch").

For a **Forge-owned unattended verification** — a launched command that must run under forge's own compatible toolchain, e.g. `forge launch run --require-control-toolchain -- forge review-loop …` or a test chain the orchestrator submits — pass `--require-control-toolchain`. It pins the workload's `PATH` to forge's own control-runtime node and **refuses before executing** if the resolved toolchain's ABI does not match, with a named message (e.g. Node 23/ABI 131 vs Node 24/ABI 137) — instead of silently depending on the ambient login-shell `PATH` selecting an incompatible node and failing deep in the suite with opaque `ERR_DLOPEN_FAILED`. A caller-supplied login shell (`bash -lc`, `zsh --login -c`) is refused under the contract, because it re-sources profile scripts that can reset `PATH` after the pin; drop `-l`/`--login`, or run the command directly without a shell wrapper. Operators running `forge launch` themselves don't need the flag — it's for the unattended path.
