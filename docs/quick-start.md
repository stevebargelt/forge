...
This installs the forge orchestrator block into `~/code/my-app/CLAUDE.md` (creating the file if needed), creates a `~/code/my-app/.forge/` directory (provisioning `model-policy.yml` from a bundled seed when absent, and provisioning/repairing `docs-surfaces.yml` — see below), scaffolds a `backlog/` directory (`stories/`, `epics/`, `ideas/`, `done/`, `notes.md`), AND installs the following:
