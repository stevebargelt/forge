...
|------|--------|---------|-------|
| Unit | `*.test.ts` (excluding the two below) | `npm run test:unit` | 1466 |
| Integration | `*.integration.test.ts` | `npm run test:integration` | 279 |
| Worktree | `*.worktree.test.ts` | `npm run test:worktree` | 85 |
| Aggregate (root) | all three above | `npm test` | 1830 |
| Shipped-claim aggregate | root + dashboard workspace | `npm run test:all` | 1857 |
