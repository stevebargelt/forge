...
- **`test`** (check name `CI / test`) — the **required** merge-gate check. Runs `npm run test:all` (unit tier + dashboard workspace, plus `npm run typecheck`) on every push and on every PR into `main`, pinned to the Node version in `.nvmrc` (24) so the better-sqlite3 native module always matches the runner's ABI. This is the merge gate: the orchestrator confirms the PR's CI check is green instead of re-running `npm run test:all` on the host before merging — see CLAUDE.md's "Merge authorization" section. Now completes in ~2.5s server-side (well under the <=60s target / <=120s ceiling), where the pre-FG-495 version took ~107s.
- **`test-extended`** — informational, **not** a required status check (`continue-on-error: true`). Runs `npm run test:extended` (integration + worktree tiers) on the same push/PR triggers, so the slow/control-plane/stress coverage still runs on every commit and is visible in the Actions UI, but a red run doesn't block merge — only `test` gates branch protection.

Running the aggregate on the host is still normal during local iteration (`forge-test --all`, or `npm run test:all` directly); what changed with FG-474 is that the merge decision reads CI's result rather than triggering a second, invisible host run of the same suite. What changed with FG-495 is that both the host run and the CI run are now fast, because the canonical gate no longer includes the integration/worktree tiers.
...
# Both slow tiers together — what CI's non-blocking test-extended job runs
