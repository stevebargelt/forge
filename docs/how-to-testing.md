...
`.github/workflows/ci.yml` has the fast `test` job plus the extended merge gate, which is now sharded across six concurrent jobs behind a fail-closed aggregate:
...
# (which runs the same coverage as six concurrent sharded/tiered jobs)
