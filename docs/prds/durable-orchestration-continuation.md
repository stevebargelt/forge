...
**Status: MET (Slice 2, FG-552). [SHIPPED 2026-07-18 (FG-552) — atomic records / bounded-retry completion disposition delivered].** Both records are now committed atomically in `src/v2/launch.ts`: the exit record is written to a sibling temp file and `renameSync`d into place inside the exit-recorder script, and `meta.json` is published exactly once via `writeJsonAtomic` (temp + rename) after the owner pid is knowable — a consumer never observes partially-written JSON as a terminal result, nor a running launch as "no such launch." The reader (`readLaunch`) no longer treats an empty or unparseable exit record as terminal: only a parseable record is authoritative, and an unreadable one falls through to owner evidence under bounded retry. The design narrative below is preserved as the accepted record.
