...
- **When `~/.forge/` is drifted or broken.** `forge upgrade` is the named remedy for the seed-drift the release check reports, and it works from a release with no dev checkout on the host — the assets come from the release itself. On a release, ask for that half by name: `forge upgrade --skip-git --skip-npm` refreshes the seeds and exits 0.
