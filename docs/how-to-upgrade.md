...
- **A `<file>.forge-pre-fg654.bak` may appear beside an adopted seed**, and upgrade names it in its output. It is the file exactly as it was before adoption, written whenever adoption replaced a protocol section whose content differed from this release's. Nothing else consumes it — keep it if you want to diff, delete it when you're satisfied.
- **A seed with a lone or hand-edited marker is refused, not guessed.** If a file has one marker without its pair, or two copies of a protocol heading, the region's boundary is genuinely ambiguous; upgrade leaves that file **untouched**, names it, and prints the exact repair. This is the one rung of adoption that needs you, and no host reaches it without having hand-edited a marker.
...
| agent protocol region | a seed's region could not be published — an ambiguous or hand-edited fence upgrade refuses to guess at (the file is left untouched and the repair is printed), or the write failed |
