...
- **Upgrade has a new pass (step 4) that maintains that region**, and it runs right after `install-seeds.sh`. Your own sections survive byte-for-byte, and your own `##` sections keep their position — the fence takes the place of the Forge sections it adopted, so what was above them stays above and what was below stays below. (One case moves, and only within your own file: a `###` of yours nested *between* two Forge sections — see the `###` bullet below.) Re-running upgrade on a current host reports the region `unchanged` and writes nothing.
