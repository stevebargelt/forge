...
1. **`git pull --ff-only`** in the dev checkout (`~/code/forge` by default; override via `--forge-repo` or `FORGE_REPO_DIR`). Does not pull if a **tracked** file in the working tree has uncommitted changes (modified/staged/deleted) — protects in-progress forge changes — and exits 1, because you asked for advancement and did not get it. Untracked files are ignored: forge's own project-provisioning creates untracked artifacts in this very checkout (e.g. `.forge/docs-surfaces.yml`), and a provisioned file must never wedge the tool that provisions it — `git pull --ff-only` itself is still the safety net if an untracked file would collide with an incoming commit. **Refused under a promoted release** — see [Execution mode](#execution-mode-what-upgrade-will-and-wont-do) below.
...
| pre-upgrade host-edit backup | the backup FAILED — operator-edited host seeds were NOT backed up, so the migration gate cannot open and the flip must not proceed until a re-run succeeds |
...
**Asset repair needs no dev checkout at all.** This is deliberate: `~/.forge` being broken or drifted is exactly when you need `forge upgrade`, so a release repairs its **forge-owned** host seeds — which since FG-777 includes `agents/`, `constraints/`, and `forge-raci.md` — from its own bundled assets even on a machine that has never cloned forge. (Until the one-time host-edit backup has run on this host, those three stay gated and a diverged copy is retained rather than repaired; see [When to upgrade](#when-to-upgrade) and the `authoredRetention` field below.) A refusal never blocks that half — steps 3 and 4 run in the same command that refused steps 1 and 2. To ask for that half and nothing else — the clean-exit release recipe — pass `forge upgrade --skip-git --skip-npm`. If the checkout is simply absent in dev mode, steps 1 and 2 report `SKIPPED` (nothing was refused and nothing failed, so the exit code stays 0).
...
```json
{
  ...
  "hostEditBackupDir": null,
  "hostEditBackedUp": [],
  "hostEditReport": [],
  "hostEditLatchPath": "/Users/you/.forge/pre-upgrade-backup/.host-edit-migration-complete",
...
| `hostEditMigration` | `"backed-up" \| "nothing-to-back-up" \| "would-back-up" \| "would-note" \| "failed" \| "not-run"` | Unresolved on `failed` only. FG-776's ONE-TIME pre-upgrade host-edit backup, run as part of step 3 before `install-seeds.sh` can flip agents/constraints/raci to always-FORCE. `backed-up`/`would-back-up` = ≥1 host authored file diverged and was (or would be) copied to `hostEditBackupDir`; `nothing-to-back-up`/`would-note` = nothing diverged, but the completion latch still gets written (or would); `failed` means nothing was backed up and the FG-777 flip must not proceed until a re-run succeeds |
| `hostEditBackupDir` | `string \| null` | The timestamped dir this run wrote diverged host files to (`$FORGE_HOME/pre-upgrade-backup/<timestamp>/`). `null` when nothing diverged, on a dry run, or when the step did not run |
| `hostEditBackedUp` | string[] | Paths (relative to `$FORGE_HOME`) actually copied into `hostEditBackupDir` this run |
| `hostEditReport` | string[] | The loud, two-case reinstate guidance (ADDED vs EDITED files) — the same lines the human `⚠` block prints, telling you how to re-express each backed-up file as a `<project>/.forge` override |
| `hostEditLatchPath` | string | The completion-latch path FG-777's gate reads. Its mere existence — not its contents — is what lets the next run's `install-seeds.sh` FORCE-overwrite agents/constraints/raci |
...
| `authoredRetention` | `"none" \| "retained" \| "not-run"` | Never unresolved. `retained` = the migration gate withheld the FORCE overwrite of ≥1 flipped seed (`agents/`, `constraints/`, `forge-raci.md`) because your copy diverges from this release's seed and the one-time host-edit backup hasn't completed on this host yet; `none` = nothing retained — either nothing diverged, or the gate has already passed and these are forge-owned like any other seed; `not-run` = the installer never ran, so nothing was inspected (not the same as `none`) |
| `authoredRetentions` | string[] | Which of the three migration-gated seeds (`agents/`, `constraints/`, `forge-raci.md`) the gate withheld the overwrite for — paths relative to `~/.forge/`, the same set the human ⚠ names. Informational, never a failure; it is the only place a script learns those files are running unrefreshed, still waiting on the one-time host-edit backup. Empty unless `authoredRetention` is `retained` |
...
### Host authored seeds are forge-owned and always upgraded (FG-777)

FORCE overwrites forge-owned files. Host authored seeds — `~/.forge/agents/<role>/CLAUDE.md`, `~/.forge/constraints/`, and `~/.forge/forge-raci.md` — are forge-owned and **ALWAYS** upgraded, the same as a runtime or workflow YAML: `forge upgrade` (FORCE) reinstalls them every time, and a bare install refreshes an absent one. Before FG-777 these three were operator-authored (FG-578): forge seeded them once and never wrote over them again. That is no longer true for any of them.

Customization lives in `<project>/.forge`, which upgrade never touches:

- an **agent** gets a project ADDENDUM — `<project>/.forge/agents/<role>/CLAUDE.md`, appended as a labeled section immediately after the always-current host base (never a replacement);
- **constraints** are an additive UNION — the host set always applies, and `<project>/.forge/constraints/` can only add or tighten, never remove a host constraint;
- the **RACI** is a full-replacement project override — `<project>/.forge/forge-raci.md`, read in place of the host RACI when present.

The always-upgrade is **gated**: before the *first* FORCE overwrite of these three categories, `forge upgrade` runs FG-776's one-time host-edit backup, which copies any host authored file you had edited to `$FORGE_HOME/pre-upgrade-backup/<timestamp>/` and prints how to re-express it as a project override (an edited file becomes a small addendum/delta, not the whole file pasted back — that would re-fork the base; a file you added is reinstated as-is). The flip refuses to run until that backup pass has completed — absent it, the three categories fall back to the pre-flip create-only + retain behavior, and upgrade tells you the migration must run first. **A genuine operator edit is never destroyed.**

### The Forge-owned review protocol

There is no region inside `~/.forge/agents/<role>/CLAUDE.md` that carries the review contract, no adoption pass, and no `.bak` file: since FG-654 the **Forge-owned review protocol** — the contract a dispatched reviewer's output is judged against — is not in that file at all. It is `seeds/agent-protocols/<role>.md`, a forge-owned category of the atomic seed generation, and compose puts it in the prompt **ahead of** your prose (host base, then project addendum). Editing your seed can never make the host read as stale; republishing the protocol can never touch a line you wrote.
...
| **embedded legacy protocol in your seed** | your `~/.forge/agents/<role>/CLAUDE.md` still contains a copy of the protocol | `forge upgrade` — see below |
...
**Since FG-777, `forge upgrade` clears this for you.** Agent seeds are forge-owned and always upgraded now (see [above](#host-authored-seeds-are-forge-owned-and-always-upgraded-fg-777)): the very next `forge upgrade` backs up your current file — embedded protocol and all — to `$FORGE_HOME/pre-upgrade-backup/<timestamp>/`, then FORCE-overwrites `~/.forge/agents/<role>/CLAUDE.md` with this release's clean, protocol-free seed, which resolves the refusal without a hand edit. Nothing is lost: the backup step prints how to re-express your prose, house rules and role notes as a `<project>/.forge/agents/<role>/CLAUDE.md` addendum, appended after — never replacing — the refreshed base. `forge doctor` names every role still in this state, so you can confirm a `forge upgrade` cleared all of them in one pass rather than discovering them one refused dispatch at a time. (You can still delete the section by hand instead — the refusal text above still offers that path, and it works exactly as before — but it is no longer the only remedy.)
...
- **The forge repo has a tracked file with uncommitted changes AND you don't want to lose those changes.** Step 1 declines to pull in this case; you'll see `DID NOT RUN (working tree has uncommitted changes in forge repo)` and the rest of the upgrade still runs. Only tracked-file changes (modified/staged/deleted) count here — an untracked file never blocks the pull. A dirty tree is **not** an operator skip — you asked for advancement and did not get it — so the run closes `INCOMPLETE` and exits 1. Commit or stash in the checkout and re-run, or pass `--skip-git` to say you meant not to pull (which exits 0). Note the seeds you install will be from your in-progress source, not from upstream. Usually fine, sometimes not.
...
git pull --ff-only
npm install
FORCE=1 ./scripts/install-seeds.sh  # refreshes forge-owned FLAT seeds, agents/constraints/RACI included (FG-777) —
                                    # but ONLY once the one-time host-edit backup has run at least once via
                                    # `forge upgrade`/`forge-dev upgrade` (this script alone never runs that
                                    # backup, so on a host that has never run a real upgrade since FG-777 shipped,
                                    # this retains your agents/constraints/RACI edits instead of overwriting them —
                                    # see "Agent seeds and the Forge-owned protocol" above)
...
- **Does not rebuild the agent Docker image by default.** If any of the image's build inputs changed, pass `--rebuild-image` to handle it in the same command (`forge-dev upgrade --rebuild-image`), or run `./docker/build.sh` separately. Both are dev-checkout operations: rebuilding runs a script from, and bakes an image out of, the checkout, so **a release refuses `--rebuild-image`** rather than build from a tree it isn't executing from. A build input is `docker/agent-dev-worker.Dockerfile` **or any file the Dockerfile `COPY`s** — currently `docker/forge-backlog-reader.mjs`, `docker/forge-backlog-bin.sh`, `docker/forge-test.sh` (the in-image test wrapper) and `docker/agent-entrypoint.sh`. Editing one of those scripts leaves the built image stale even though the Dockerfile's own mtime never moved. Forge will keep using the old image until you rebuild — usually fine, but watch for breakage if the image picked up something load-bearing (e.g. a new tool in PATH, or a changed test-wrapper exit contract).
- **Judges image staleness by content, in both modes (FG-543).** `docker/build.sh` hashes the Dockerfile plus every file it `COPY`s (content, not path or mtime) and stamps the result on the built image as the `forge.build-inputs.digest` label — even on a fully-cached build, so a cached rebuild still records the current digest and clears a stale flag. `forge doctor` (and the release check at the end of `forge upgrade`) recomputes that same digest from the executing tree and compares it to the label: `STALE` when they differ, or when the label is absent entirely (an image built before this label existed, or by hand — it stays `STALE` until rebuilt once, which stamps it). Comparing content rather than timestamps means the two false-positive classes that used to misfire are gone: a fully-cached rebuild (new image ID, unchanged `created` timestamp) and a git checkout with fresh mtimes but unchanged bytes. It also means the check no longer needs a dev/release split — a release tree is materialized by copy, which preserves content even though it doesn't preserve timestamps, so a release's build inputs hash the same as what was actually built and read as not stale. Rebuild advice stays mode-aware regardless: a release host is pointed at `forge-dev upgrade --rebuild-image`, since a release can't rebuild its own image.
- **Pins Claude Code in the image, and checks it can serve your models (FG-804).** `docker/agent-dev-worker.Dockerfile` installs Claude Code at `ARG CLAUDE_CODE_VERSION` (same pattern as `CODEX_CLI_VERSION`), not "latest": an unpinned install froze at whatever the first build's cached layer resolved, and nothing refreshed it. To move the CLI, bump the ARG and rebuild — editing the Dockerfile changes the build-input digest, so `forge doctor` reports the image `STALE` until you do. Separately, `forge doctor` (and the release check at the end of `forge upgrade`) has a **`claude CLI version`** row that reads `claude --version` from inside the image and compares it to the minimum CLI version each model needs, declared in one table, `CLAUDE_CLI_FLOORS` in `src/v2/claude-cli-floor.ts` (today: `claude-opus-5-5` needs Claude Code >= 2.1.280). Every model reachable through a runtime that invokes `claude` — its `models:` aliases and every `model_profiles` entry bound to it — is checked. It is a **blocking FAIL** when the image's CLI is too old for any of them, naming the model, where it is configured, the required version, the image's version, and the rebuild command (if the Dockerfile pin is itself below the floor, bump it first). If the version cannot be read while a floored model is configured, that is also a FAIL (the floors cannot be verified); with no floored model configured it is a warning. The row is skipped when the image or its `claude` CLI is absent — those have their own rows.
