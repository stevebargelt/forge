...
| `routingPolicy` | `"recompiled" \| "would-recompile" \| "no-raci" \| "failed"` | Unresolved on `failed`. `failed` = the promoted runtime rejected the host RACI, so the stale compiled policy was **invalidated** (quarantined to `routing-policy.yml.quarantined`, or removed) and routing is fail-closed until the RACI compiles — never left silently authoritative. `no-raci` is fine — no derived artifact to keep in lockstep |
| `routingPolicyError` | `string \| null` | The compiler's verbatim reason for the rejected RACI construct — the same named refusal the human warning prints. `null` on every non-`failed` path. If the stale policy could not be neutralized on disk, this also carries that (the policy is still authoritative; remove it by hand) |
