...
Nine roles are covered: the five `red-*` discovery lenses plus `engineer`, `documentation-maintainer`, `review-rechecker`, and `shipping-reviewer`. A covered role is REFUSED at dispatch — by name, before any container or task dir exists — in five states, all of which `forge doctor` also reports as a readiness `[FAIL]`:
