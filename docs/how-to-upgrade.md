...
- **Upgrade has a new pass (step 4) that maintains that region**, and it runs right after `install-seeds.sh`. Your own sections survive byte-for-byte and in place. Re-running upgrade on a current host reports the region `unchanged` and writes nothing.
...
- **An ambiguous seed is refused, not guessed.** Three shapes reach this rung: one marker without its pair, two copies of a protocol heading, or a seed carrying only *some* of the region's headings — that last one means a section of yours may simply share a name with a Forge one, and forge will not delete your content on that guess. Upgrade leaves the file **untouched**, names it, and prints the exact repair. A **symbolic link** where a seed (or its `.bak`) belongs is refused the same way and for the same reason: publishing through it would write outside the seed tree.
- **Your own `###` subsections nested under a Forge `##` heading survive adoption**, in the live file — not only in the `.bak`. Adoption replaces the sections the region owns, and a subsection heading the region does not carry is yours.
...
| agent protocol region | a seed's region could not be published — an ambiguous fence, a heading collision or a symlinked seed upgrade refuses to guess at (the file is left untouched and the repair is printed); the executing release carries no fenced seed for one of the covered roles, so nothing was published for it; or the write failed |
...
| `agentProtocol` | `"published" \| "already-current" \| "would-publish" \| "not-run" \| "needs-repair" \| "incomplete" \| "failed"` | Unresolved on `needs-repair`, `incomplete` and `failed`. Step 4, the Forge-owned protocol region inside each agent seed. `needs-repair` = at least one seed is yours to fix — an ambiguous marker fence, a heading collision forge refused to guess at, or a symbolic link where the seed belongs — and those roles refuse at dispatch until you fix it. `incomplete` = the executing release carries no fenced seed for a covered role, so nothing was published for it; re-running here converges nothing (reinstall the release). `failed` = the publish threw (permissions, disk); roles published before it are current, the rest are not. `not-run` = the asset install never ran |
| `agentProtocolLines` | string[] | The per-role publication report — what was adopted, appended or replaced, every `.forge-pre-fg654.bak` written, and every seed left untouched pending a hand repair. The same lines the human surface prints |
