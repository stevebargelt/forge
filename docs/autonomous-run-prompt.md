...
- For any ticket producing code or operating-policy changes (campaign lane or direct execution), run this sequence after implementation. The coordinator enforces every step when you drive one through `forge review`:
  ...
  10. A selected reviewer that crashes, times out, or produces missing/malformed output leaves the review INCOMPLETE — not a clean result. An absent lens clears only by retrying it, amending the contract through its approving authority, or an authorized acceptance that NAMES the missing evidence (`forge review accept-lens <review-id> <lens> --operator --missing-evidence "..." --rationale "..."` — an operator decision, so a hard-stop in an autonomous run). One absence is outside all three (FG-654): a lens the dispatch REFUSED for `stale_protocol` — that reviewer's Forge-owned protocol region was absent, unfenced, ambiguously fenced, or behind the running release — is not acceptable-away and does not clear on a bare retry; run `forge upgrade`, then re-run discovery, which is self-service rather than a hard-stop. An ambiguous fence is the exception upgrade will not fix for you: it names the seed and the hand repair, and re-running upgrade before you do it changes nothing. Never synthesize a pass or an empty finding set.
