...
- For any ticket producing code or operating-policy changes (campaign lane or direct execution), run this sequence after implementation. The coordinator enforces every step when you drive one through `forge review`:
  ...
  5. Send ALL current UNRESOLVED fix_now findings to ONE fixer in ONE immutable, candidate-bound FixBatch naming each finding ID with its evidence. Not one fixer per round. A fix_now already resolved at the current candidate is not re-dispatched and keeps its resolution (FG-649/RF-8).
  ...
  10. A selected reviewer that crashes, times out, or produces missing/malformed output leaves the review INCOMPLETE — not a clean result. An absent lens clears only by retrying it, amending the contract through its approving authority, or an authorized acceptance that NAMES the missing evidence (`forge review accept-lens <review-id> <lens> --operator --missing-evidence "..." --rationale "..."` — an operator decision, so a hard-stop in an autonomous run). One absence is outside all three (FG-654): a lens the dispatch REFUSED for `stale_protocol` — that reviewer's Forge-owned protocol was unpublished, missing from the published generation, torn, or behind the running release — is not acceptable-away and does not clear on a bare retry; for those generation-side shapes run `forge upgrade`, then re-run discovery, which is self-service rather than a hard-stop. ONE shape is an exception upgrade will not fix for you, so read the refusal before assuming a second upgrade helps: a release carrying no protocol for a covered role is the executing tree's problem rather than this host's — reinstall the release, since publication refuses the same missing file. The other shape — an agent seed still carrying an EMBEDDED copy of the protocol — is cleared by the next `forge upgrade` itself since FG-777 (that seed is forge-owned and force-overwritten, gated on the one-time host-edit backup), though hand-deleting the named section still works. Never synthesize a pass or an empty finding set.
- Do NOT commit the fixer's output by hand, and never re-anchor a review's candidate by hand (FG-649). Committing the IMPLEMENTATION range is yours; the fix cycle is the coordinator's — `forge review continue` commits the fixer's declared files and records the post-fix candidate automatically, so recheck examines the tree the fixes are actually in. A hand-commit is what created the FG-649 loop: the coordinator read HEAD before that commit existed, pinned the candidate pre-fix, and the rechecker then honestly reported still_present against already-fixed code with no mechanical way out. The coordinator commits only the paths the batch's results declared and refuses by name when the tree disagrees — `fix_cycle_declared_changes_absent`, `fix_cycle_tree_dirty_outside_declared_scope` or `fix_cycle_commit_raced` (the commit did not land as authored; it is not adopted) means the fix cycle did NOT complete (nothing recorded, candidate unmoved): resolve the named condition and re-run continue, do not work around it or journal it as informational. A cycle that succeeds but NOTES declared paths the worktree never moved is the opposite case and is informational — the commit carried what moved, and the recheck adjudicates the claim.
- `forge review continue` acts on the workspace the review recorded, not your cwd (FG-649). `start` persists the resolved `--project` on the review row and a later `--project` is RECORDED before use (a `--dry-run` preview resolves and refuses identically but binds nothing); an unbound or unusable workspace refuses by name and writes nothing (`review_workspace_unbound`, `review_workspace_unusable`, `review_workspace_identity_mismatch`), each naming `--project`. Supply the path — do not cd somewhere and re-run.
...
- A retained failed container may still hold injected secrets (auth tokens, API keys) in its environment or filesystem. Forge now retires it automatically once its diagnostic window passes (`7 days` by default for a failed/ambiguous outcome, daemon-free — see `docs/operator-surface-addons.md` → Automatic terminal-resource cleanup, FG-590), but do not rely on that window alone once an investigation is complete: run `forge ops reap-containers` (or `forge ops cleanup`, or set `FORGE_CONTAINER_RETENTION=off`) rather than leaving a secret-bearing container inspectable for the full window.
- Use `forge ops reap-containers` or `forge ops cleanup` to clean up retained containers once an investigation is complete, rather than removing them by hand or waiting out the automatic retention window.
