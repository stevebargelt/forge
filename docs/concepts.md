...
`forge retry <task-id>` takes a `failed` task and mints a fresh pending row for a second attempt: a new task id (so a new task dir — never a reuse of the failed attempt's `result.json` or staged auth state), the same phase/role/inputs, `parentId` pointing back at the failed task for lineage, and `inputs.previous_failure` carrying the failure kind and error. The failed row itself is kept as-is for audit. Whether the retry is allowed at all is `retry-policy.ts`'s call, per `failure_kind` (see [Orphaned task recovery](#orphaned-task-recovery)); `--force` overrides a non-retryable kind.
