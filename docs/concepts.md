...
`forge recover <id>` takes a task or run id. Read-only by default: it recomputes a fresh evidence view (worktree vs. shared-project-dir source, changed files, a valid `result.json` or a stdout-inferred result) and prints a recommended next command; `--json` is the full structured surface, where `recommendation` is the rendered line and `recommendationCommands` (FG-507) is that same recommendation as an ordered array of the individual commands to run — usually one, two for the cancel→retry sequence below.
