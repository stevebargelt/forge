...
- The campaign must be in `planned` or `paused` state; calling `approve` on any other status is an error. `paused` is valid (FG-442) because `approve` is also the confirm/override point after a [lane escalation](#lane-escalation) — the same command, not a new one.
