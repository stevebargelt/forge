...
2. else a green required CI check (`CI / test`, from `.github/workflows/ci.yml`) at the exact sha, read via the GitHub check-runs API — any completed failure on that sha disqualifies it outright (a same-sha pass/fail disagreement across re-runs is treated as a flake signal, never covering evidence), and a still-running check is pending, never covering.

**Fails closed:** no evidence, a different sha, a different command, or a pending/failing/disagreeing check all return nothing, so the caller falls back to actually running the gate — never a synthetic pass.
