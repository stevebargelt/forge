...
`forge recover <id>` takes a task or run id. Read-only by default: it recomputes a fresh evidence view (worktree vs. shared-project-dir source, changed files, a valid `result.json` or a result recoverable from stdout — structured or FG-337-inferred, per [Recovering a result from stdout](#recovering-a-result-from-stdout)) and prints a recommended next command; `--json` is the full structured surface, where `recommendation` is the rendered line and `recommendationCommands` (FG-507) is that same recommendation as an ordered array of the individual commands to run — usually one, two for the cancel→retry sequence below.
...
Through FG-357/FG-424 the gate ran against the publish target *after* the merge had already landed on it, so a gate failure could only report a bad merge that was already published — and because forge's run lock is per-`runId` and never per-project, two runs on one project could interleave their merges and gates against a moving HEAD. The **integration publisher** (FG-425, FORGE-DEC-026) inverts that ordering: it validates a candidate commit in a throwaway worktree and publishes only what passed. The publish target now fast-forwards to a fully-validated commit or does not move at all. This is worktree-mode only (`FORGE_WORKTREES=1`); non-worktree runs bind-mount the shared project dir, do no merge, and run no gate, exactly as before.

Every publication attempt follows the same shape — single-primary (FG-352) and fan-out (FG-353) both converge on the one publisher:

1. Record the intent and the target's current `baseSha`, durably, **before** any mutation.
2. Create a fresh, per-attempt candidate worktree under `~/.forge/worktrees/publications/<attemptId>-r<rebuild>` — never pooled, never reused (a crashed attempt is abandoned; a retry gets a new one).
3. Merge the task branch (single-primary) or the children's integration branch (fan-out) there, producing candidate commit `candidateSha`.
4. Validate that candidate **in that worktree, never against the target**: the integration gate (the project's own `npm run test:unit`), then the reds and any review gate. Publication is conditional on all of it — a red rejection now means nothing ever reached the target, which is what makes a red a gate rather than a notification.
5. Queue on the project's FIFO publication lane, take the short publication mutex, confirm the target is still at `baseSha` (compare-and-swap), and fast-forward it to the **recorded** `candidateSha` — never a branch tip, never current worktree state. The mutex is held across the ref update alone, never across validation.
...
If the target moved off `baseSha` before the compare-and-swap, the attempt rebuilds on the new base and re-runs the *whole* validation set (reds included) once. A second move parks with `publish_base_churn`. Because the FIFO lane means forge-owned attempts cannot move each other's base, repeated churn is evidence that something *outside* forge is writing to the target — find that writer; raising the rebuild bound only buys more rounds of losing the same race.
...
Four `failure_kind`s are specific to publication. All four are terminal, and on all four **nothing was published**:

- **`dirty_publish_target`** — the target has uncommitted tracked changes, or the candidate would overwrite an untracked file. Refused *before* any mutation: forge never stashes, resets, cleans, or checks out over operator-owned state. Commit or stash your changes, then re-run.
- **`publish_base_churn`** — the target moved off the validated base twice (above). Evidence is preserved; look for the external writer.
- **`publication_refused`** — the candidate did not descend from the base it was validated against, or its checkout could not be applied and the ref advance was rolled back. The target is byte-for-byte where it started.
- **`lane_taken_over`** — the attempt's lane lease lapsed and a later attempt claimed the lane. A retry simply enqueues a new attempt with a fresh worktree.
