...
- **`forge review-loop`'s verification phase** (`buildReviewLoopDeps.verify` in `src/cli/commands/review-loop.ts`) — only when the worktree is clean (a dirty tree never reuses, since a dirty diff under review wouldn't match what any recorded/CI evidence actually covers). On reuse it records what covered it (`reusedEvidence`, rendered in the loop's run note) instead of executing anything; on a dirty tree or no evidence it falls back to `runVerification` (`src/v2/review-loop.ts`), which runs `typecheck`, `test`, and — since FG-500 — `test:extended` too, whenever each script is present in `package.json`. This fallback list is driven purely by script presence, not by `deriveRequiredGateList`, so it picks up the extended tier regardless of whether `requiredHostGate` is still the project default.
