...
Four `failure_kind`s cover recovery from a lost container or a lost fanout wave, and `forge recover <id>` is the operator-safe way to inspect and act on any of them: one lost-container kind with no evidence of persisted work, two lost-container kinds where work may have persisted, and one for an orphaned fanout wave.
...
**`host_verification` in the packet.** The collector reads matching evidence from the host-verification store when the ticket has a `closedCommit`, applying the passing-row aggregation described above: a covering row with `exit_code 0` → `hostVerified: true`, even if an earlier covering row failed; covering rows exist but none passed → `hostVerified: false`; no covering evidence → `hostVerified: null` (unknown). The `host_verification` check's `detail` field carries `gate`, `command`, `exit_code`, `commit`, and `recorded_at` from the evidence row (plus `earlier_covering_failures` when a prior covering failure is retained as audit history alongside a later covering pass). `doneAudit.outcome: "pass"` is reachable for real items once matching host-verification evidence has been recorded via `forge record-host-verification` (see [Recording host-verification evidence](#recording-host-verification-evidence)).
...
**Evidence-reconciling a manually-driven item (FG-441).** Before re-parking a manually-driven `awaiting_gate` item — `lifecycleStatus: awaiting_gate`, no `blockerKind`, and a `runId` on record — resume evidence-reconciles it against the same durable evidence Shape 1 of [Reconcile](#reconcile) uses: `collectReconcileEvidence` + `evaluateReconcileEvidence` (ticket `done`, `closedCommit` reachable on the base branch, a passing host-verification row, and the run's own authoritative-review outcome resolved to `pass` or superseded by a qualifying force-advance). This is the events-aware evaluator that reads the run's verdict/gate history. Reconcile's shape-2 out-of-band evaluator stays events-blind for lane-delivery evidence, but as of FG-458 it also composes this same authoritative-outcome fact for a shape-2 item that has a `runId` (see [Reconcile](#reconcile)) — so the two paths cannot disagree on whether the run's own authoritative review is unresolved. Such an item can have been driven to completion entirely outside the resume loop — merged, closed, host-verified — while it still sits parked at its workflow gate; previously resume re-parked it forever. Eligible → the item is atomically marked `complete`/`shipped`, a `campaign_item.evidence_reconciled` audit event is recorded, and dependents held only on this item are released. Ineligible → resume refuses to ship it and falls through to the unchanged re-park, reporting the missing facts to stderr and as a durable `campaign_item.evidence_reconcile_refused` audit event — never an optimistic ship.
...
**Shape 2 evidence.** Still does not consult the item's own run events for lane-delivery evidence — the delivering work happened outside that run, so its run's event/verdict history (present, absent, or failing) cannot substitute for real out-of-band delivery evidence. But when the item has a `runId`, reconcile now also composes the run's own authoritative-review outcome (FG-458) — the same fact `forge campaign resume`'s FG-441 evidence-reconciliation checks for this exact shape (see above) — from the two authoritative-outcome codes in the shape-1 table (`no_authoritative_verdict_or_force_advance_event`, `latest_authoritative_verdict_is_fail_with_no_later_pass_or_force_advance`), surfaced here as `run_evidence:<code>` and evaluated before the lane-evidence capture gate below so an unresolved fail refuses without spending a gate run on it. An item with no `runId` was never attached to a run in the first place (the legitimate FG-443 out-of-band delivery case) and keeps the pure events-blind path. This closes the gap where `resume` refused a failed-review item that `reconcile` would otherwise have shipped — the two paths now agree specifically on the authoritative-outcome axis; the remaining evidence below (ticket/commit/lane) is still derived independently by each evaluator, and a residual divergence there for docs-only items is tracked separately as FG-460. An item is marked complete only when ALL of the following hold:
