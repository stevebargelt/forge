...
- A **workflow-step** task — its `phase` is a declared step id of its run's workflow — is dispatched by the ready queue, so retry prints the `Next: forge next <run-id>` pointer and stops there.
- An **ad-hoc** task is one the workflow runner never scheduled. Two shapes count: a task on a synthetic `invoke` run or an `invoke_chain` run (campaign quick lanes chaining plain invokes), and a task attached to a *real* workflow run via `forge invoke --run <run-id>`, whose `phase` is not one of that workflow's step ids. Both are invisible to `computeReadyQueue`, which walks workflow steps — so the old `forge next` pointer stranded the new row forever, waiting on a queue that would never see it. `forge retry` now re-dispatches an ad-hoc row itself, immediately and in-process, with invoke semantics, and prints no `forge next` pointer. The ad-hoc test is keyed on the workflow's actual step ids, never on `phase === "task"` alone — a real workflow may legitimately declare a step called `task`.
- **Unknown** is the third state, and it is what keeps the other two honest. The second ad-hoc shape can only be ruled in or out by reading the run's workflow, so a workflow YAML that has gone missing or no longer parses leaves the question unprovable. `taskDispatchKind` (`src/v2/run-kind.ts`) returns `{ kind: "unknown", loadError }` there rather than collapsing the case into "workflow step" — that collapse is exactly what minted stranded rows, since retry would create a pending task and point at a `forge next` that never sees it. Only pipeline runs can reach this state; an `invoke` or `invoke_chain` run is decisively ad-hoc without a load attempt. `forge retry` is the one caller that interprets `unknown`, and it refuses (below).
...
Retry refuses a second way, one step earlier, when the discriminator itself comes back `unknown`: `RetryWorkflowUnloadableError`, exit 1. Forge cannot tell whether the task is a step of the run's workflow — which `forge next` would re-dispatch — or a `forge invoke`-attached row, which `forge next` never dispatches and which would strand as a pending row, so it declines to guess in either direction. Like the refusal above it happens **before any write**: no pending row, no `forge next` pointer, nothing to clean up, and the message says so. It names the workflow and quotes its load error verbatim, then offers both recoveries, since only the operator knows which task this was — restore the workflow YAML and re-run `forge retry <id>`, or, if the task was ad-hoc, re-dispatch it directly. The second recovery is a ready-to-run `forge invoke` command when the row recorded `inputs.task` text; when it didn't, the command comes without a `--task` and says so plainly, telling you to supply the original task yourself rather than implying the row still holds it.
