...
- else — no evidence, a different sha, a different command, a red/pending/absent sibling job, an enumeration failure, or a pending/failing/disagreeing check (fails closed) — it runs the required gate for real, in `projectDir`, at its current HEAD, and records the result with `source: "host"`, exactly as before FG-474:
