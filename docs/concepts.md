...
`resume` itself still does neither automatically — both verbs are explicit operator calls.
...
`campaign_system` is not a classification the way `auth` and `infrastructure` are — it is a placeholder. `reconcileTerminalOutcome` lands *any* non-`complete` run on it, so an overnight transient blip (an idle timeout, a container crash, a killed driver) that abandoned a run is indistinguishable, **at the item level**, from a genuine campaign-system fault. Retry therefore refuses to read the item's `blockerKind` as evidence of anything, and probes the underlying run instead.
...
The probe is fail-closed in every branch: a missing or unknown run, a `complete` run, no failed primary task, or *any* failed primary that classifies to a non-transient kind (including a mix of transient and non-transient) all refuse, naming what was missing or non-transient and the verb that actually applies:
