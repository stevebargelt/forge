...
- **Result missing after a clean exit** — `containerExitedEventObserved: true` with `dockerExitCode: 0`: the container is confirmed to have exited cleanly, it just never wrote a usable `result.json`. Distinct from a disappeared container — the exit itself isn't in question, only the result.

`forge show <id>` prints a `container evidence:` line (plus any `missing:` gaps) for a failed task, and `forge show <id> --diagnostic` prints *only* that causal-evidence block — the four states above, whichever applies, plus the explicit missing-evidence list (`--diagnostic --json` for the same data as structured output). `forge status <run-id>` and `--json` render the same evidence per failed task. `forge ops check`'s `orphaned_work_may_persist`-family incidents (see [Orphaned task recovery](#orphaned-task-recovery) below) also name the same confirmed-vs-disappeared distinction — read off the `OrphanEvidence` tuple's own `containerExitedEventObserved`/exit-code/signal/`OOMKilled` fields rather than the `containerEvidence` record itself, since `ops/detect.ts` sits below `cli/` and deliberately doesn't import from `show.ts`.

**Retention for investigation.** By default (`docker-exec.ts`'s `shouldRetainContainer`), a task container that exits non-zero is left running-but-stopped instead of being removed, so it stays inspectable for `forge show --diagnostic` / `docker inspect` after the fact; a clean (0) exit is always reaped immediately, since there's nothing to investigate. Set `FORGE_CONTAINER_RETENTION=off` to disable retention entirely (e.g. a disk-constrained host) — every exit is reaped regardless of code. Retention never blocks a retry: `forge retry` reaps the failed task's retained container before dispatching the new one (the new task gets its own `forge-<newTaskId>` container name, so there's no collision either way). A retained failed container may still hold injected secrets in its environment or filesystem, and there is no automatic time-bound reaper, so cleaning it up once an investigation is done is the operator's responsibility: `forge ops reap-containers` scans failed tasks' retained containers and removes them (`--dry-run` to preview, `--older-than-minutes <n>` to only reap containers whose task completed at least that long ago, `--project <dir>` / `--all` to scope, `--json` for structured output). It's read-only over the DB — a `running` task's container is never a candidate — and the removal itself is best-effort: a docker error is reported as "not confirmed gone" and left for a later sweep, never treated as a crash.
