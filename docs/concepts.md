...
**Fails closed:** no evidence, a different sha, a different command, a project whose own CI content doesn't demonstrably run the required command, a job-enumeration failure, a red/pending/absent sibling job in the matched workflow, or a pending/failing/disagreeing check all return nothing, so the caller falls back to actually running the gate — never a synthetic pass.
