...
`findCoveringGateEvidence(ticketId, projectDir, sha, command)` (`src/store/host-verifications.ts`) answers that question for an exact `(projectDir, sha, command)` triple. Command match is **exact** — every row is written with the single canonical `requiredHostGate` string as both `gate_name` and `command`, so a row recorded under a narrower command (e.g. `npm run test` vs. the required `npm run test:all`) never satisfies it. It checks, in order:

1. an existing **passing** `host_verifications` row at the exact sha + exact command;
2. else, only once this project's own CI is verified to actually run the required command: a green required CI check (`CI / test`) at the exact sha, read via the GitHub check-runs API — any completed failure on that sha disqualifies it outright (a same-sha pass/fail disagreement across re-runs is treated as a flake signal, never covering evidence), and a still-running check is pending, never covering.
...
**Fails closed:** no evidence, a different sha, a different command, a project whose own CI content doesn't demonstrably run the required command, or a pending/failing/disagreeing check all return nothing, so the caller falls back to actually running the gate — never a synthetic pass.

Two consumers consult this before running the deterministic suite themselves:

- **`forge review-loop`'s verification phase** (`buildReviewLoopDeps.verify` in `src/cli/commands/review-loop.ts`) — only when the worktree is clean (a dirty tree never reuses, since a dirty diff under review wouldn't match what any recorded/CI evidence actually covers). On reuse it records what covered it (`reusedEvidence`, rendered in the loop's run note) instead of executing typecheck/test; on a dirty tree or no evidence it runs exactly as before.
- **`forge campaign reconcile`'s host-verification capture** (`runAndRecordHostVerification` in `src/campaign/reconcile-collect.ts`) — see [Automatic host-gate capture](#reconcile) below.
...
**`host_verification` in the packet.** The collector reads matching evidence from the host-verification store when the ticket has a `closedCommit`, applying the passing-row aggregation described above: a covering row with `exit_code 0` → `hostVerified: true`, even if an earlier covering row failed; covering rows exist but none passed → `hostVerified: false`; no covering evidence → `hostVerified: null` (unknown). The `host_verification` check's `detail` field carries `gate`, `command`, `exit_code`, `commit`, `recorded_at`, and `source` (`'host'` or `'ci'` — FG-474) from the evidence row, plus `ci_url` when `source` is `'ci'` and a URL was recorded (plus `earlier_covering_failures` when a prior covering failure is retained as audit history alongside a later covering pass). `doneAudit.outcome: "pass"` is reachable for real items once matching host-verification evidence has been recorded, either manually via `forge record-host-verification` or automatically — a real host run or a reused/CI-sourced row from `forge campaign reconcile`'s capture step (see [Recording host-verification evidence](#recording-host-verification-evidence) and [Evidence reuse for the deterministic gate](#evidence-reuse-for-the-deterministic-gate-fg-474)).
...
**Automatic host-gate capture (FG-440, evidence reuse added FG-474).** Before evaluating shape-1 evidence for a scope-blocked item, reconcile checks whether a *covering* host-verification row already passed for the item's ticket at the configured `requiredHostGate`. A row covers the item when `closedCommit` is an ancestor of the row's tested commit **and** that tested commit is itself reachable on the base branch — not an exact-sha match, since the gate always runs at `projectDir`'s current HEAD, never a checkout of `closedCommit` itself. If no covering row has already passed, reconcile calls into the capture step (`runAndRecordHostVerification`, `src/campaign/reconcile-collect.ts`), which — before actually executing anything — first checks for the narrower, EXACT-sha covering evidence described in [Evidence reuse for the deterministic gate](#evidence-reuse-for-the-deterministic-gate-fg-474) above, for the current HEAD:

- a passing `host_verifications` row already recorded at this exact HEAD sha + the exact required-gate command → reused as-is (`status: "reused"`), no exec, no duplicate row written;
- else a green required CI check (`CI / test`) at this exact HEAD sha → recorded as a fresh row with `source: "ci"` and `ci_url` set to the check's details URL, again with no exec;
- else — no evidence, a different sha, a different command, or a pending/failing/disagreeing check (fails closed) — it runs the required gate for real, in `projectDir`, at its current HEAD, and records the result with `source: "host"`, exactly as before FG-474:
