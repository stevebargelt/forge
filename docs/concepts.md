...
Two failure modes, surfaced differently — but both emit `task.failed`; the machine-readable `failure_kind` in the event payload distinguishes them. **Container crash**: container exited non-zero with no result JSON (Docker issue, credential failure, or an OOM/kill that couldn't be positively confirmed as such) → `failure_kind: "container_crash"` — unless the runtime's stdout carries a provider/model error (invalid model, quota, 4xx), in which case it's attributed as `failure_kind: "model_error"` with the cause (#228). An OOM is `container_crash` only when it *isn't* positively identifiable as one: when reconcile finds a container gone and `docker inspect` confirms `OOMKilled` or exit code 137, that death is classified more specifically as `failure_kind: "oom_killed"` instead (FG-455 p4) — see [Orphaned task recovery](#orphaned-task-recovery). **Agent failure**: container exited 0 and wrote a result JSON with `status: "failed"` and an error reason → classified per context. Different follow-up actions. (There is no `task.crashed` event type — the failure taxonomy lives in the `task.failed` payload, not in separate event types.)
...
- **`oom_killed`** (FG-455 p4): the container is gone and `docker inspect` positively identifies the death as an OOM kill or exit code 137/SIGKILL — a more specific cause than the generic orphaned kinds. It takes precedence over `orphaned` / `orphaned_work_may_persist` whenever reconcile has this evidence, even over a dirty worktree — but a recoverable stdout result still outranks it (the task completes instead of failing). Exit 137 alone, without a confirmed `OOMKilled` flag, may also be an external kill rather than an OOM; `forge show`'s recovery message reflects that uncertainty ("possibly OOM or an external kill"). Like `orphaned_work_may_persist`, it's **not** retryable without `--force` (see `retry-policy.ts`) and is continuable via `forge recover <id> --continue` when there's persisted work to adopt. `forge ops check` flags an `oom_killed` task whose worktree is dirty — a clean-worktree `oom_killed` has no persisted work at risk, so it raises no incident.
