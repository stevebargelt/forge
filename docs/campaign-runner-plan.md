...
- **FG-473** (filed, not yet fixed): a `quick_implementation` (invoke-chain) item delivering a genuine code change can never complete via `forge campaign reconcile`/`resume`. The out-of-band composition folds in the run's Fact-5 authoritative-outcome contribution for any item with a `runId` (`authoritativeOutcomeContribution` in `reconcile-outofband-evidence.ts`), but an invoke-chain run produces zero authoritative verdicts — there is nothing for a force-advance to supersede — so Fact 5 is structurally unsatisfiable for this lane no matter how legitimately the work shipped. The offending item wedges the whole sequential campaign at that step.
