...
- integrated output passes integration validation before it is published, and before campaign-level success;
- campaign reports explain what ran in parallel and how it was integrated.

Prerequisites already filed: **FG-410** (`updateCampaignItem`'s read-merge-write is lost-update-unsafe under concurrent writers and must land before parallel lanes go live) and **FG-424**/**FG-425**/**FG-426** sharpen the shared gate/lock surface that parallel lanes will contend on — **FG-424** (the integration gate distinguishes real test failures from infra/platform failures) and **FG-426** (classify `integration_failed` as a scoped item blocker) have landed. **FG-425** was originally scoped as per-`projectDir` gate locking; that design was abandoned and it was rebuilt as the **serialized integration publisher** (FORGE-DEC-026), which orders *publication* per canonical project identity instead of locking the gate — worker execution stays parallel, which is exactly what parallel lanes need. See [Integration publisher](concepts.md#integration-publisher).
...
- **FG-490** (2026-07-07) — delivered: a thrown `runNext`/`startRun` mid-drive used to strand the campaign at `running` with no CLI path back (F7 in the 2026-07-06 review) — live evidence this session, when a killed build driver left run-fg-485-7bfcb2's campaign stuck `running` with no live process and needed a manual `forge campaign pause` to unstrand. The executor now catches the throw, records a durable `campaign_item.drive_error` audit event (raw error, before any state change — the only audit trail under a cron/service invocation where stderr may not be captured), parks the in-flight item, transitions the campaign `running` → `paused`, and rethrows the original error wrapped with resume guidance; a failure in the park itself is swallowed so the original drive error always propagates unmasked. The two throw shapes park deliberately differently: a `runNext` throw has a real, active run behind it, so it parks `awaiting_gate`/no-`blockerKind` — the same recoverable shape FG-485's liveness-first reattach re-drives on the next `resume`. A `startRun` throw has no run ever dispatched, so it parks directly at its true terminal shape instead — `failed`/`blocked`/`blockerKind: infrastructure`, with a terminal synthetic run row for traceability — making FG-489's `forge campaign retry <campaign-id> <ticket-id>` (once paused), then `forge campaign resume`, the recovery path; the first-round fix parked this shape at `awaiting_gate` too, which review round 2 caught as a projection mismatch against the terminal synthetic run row (`forge campaign retry` refuses a non-retryable `blockerKind`), corrected to park terminal directly. `forge campaign start`/`resume` also render this failure class as a structured result instead of a bare uncaught error (review round 1 finding) — `--json` gets `stopReason: "drive_error"` with `campaignId`, `ticketId`/`runId`, `error`, and `guidance`; human output keeps the wrapped message text. Documented in [Start](concepts.md#start-sequential-execution) and [Resume](concepts.md#resume).
- **FG-502** (2026-07-09) — delivered, three related fixes: (1) **campaign reconcile shape 3** — a `failed`/`blockerKind: campaign_system` item (previously ship-able only via `forge campaign abandon`, even when genuinely delivered) is now recoverable by `forge campaign reconcile`, evaluated against the identical shape-2 out-of-band evidence bar and recorded with a distinct `campaign_item.campaign_system_reconciled` audit event, kept apart from `campaign_item.out_of_band_reconciled` so the audit trail can tell the two recovery causes apart. `forge campaign show`/`report` preview this per-item via a mirrored (not shared-import) `campaignSystemEligible` field / `campaign-system-recoverable:` line, matching the FG-444 `outOfBandEligible` pattern. (2) **review-loop fixer scope guard** — a fixer round touching a disallowed path no longer hard-reverts the WHOLE round: `backlog/`/ticket-closeout paths stay unconditionally out of scope, but `docs/`/`learnings/`/`README*` paths are now scoped to the loop-start reviewed commit range (fixed for the whole loop) and only the disallowed paths within a round are selectively reverted — in-scope code/docs changes from the same round survive and commit, with reverted paths surfaced as closeout-guidance-style findings in the run note. A revert that can't be verified clean aborts the round fail-safe (`scope_guard_revert_failed`) rather than risk staging an unverified tree. (3) **review-loop reviewed-tip trust** — the final report and closeable verdict now name the reviewed tip sha and check its exact reachability from the locally cached remote-tracking/PR ref (`git merge-base --is-ancestor`, no implicit fetch, no ahead-count substitute). The candidate order deliberately differs from `claude.ts`'s statusBanner ahead-of-origin helper: `resolveRemoteRef` tries `@{u}` before `origin/HEAD` — the opposite order — because it's answering a different question (exact PR-tip reachability, not an ahead-count against the default branch); see the FG-502 comment in `review-loop.ts` for why swapping the order back to match `claude.ts` would reintroduce a false-negative-against-main. A closeable verdict is withheld (`not closeable`, non-zero exit) when the tip is local-only (names the unpushed commits) or when no remote-tracking ref resolves at all — never silently applied to a local-only tip. **Superseded by FG-514 (2026-07-10) — the trust check described in (3) is no longer current behavior.** Ancestry was one-directional against a ref that was never refreshed, so a remote head carrying commits the reviewer never saw still read as trusted. The check now performs a bounded single-refspec fetch of the remote-tracking ref (`git fetch --quiet --no-tags`, 20s timeout, `GIT_TERMINAL_PROMPT=0`) and requires **equality** — the reviewed tip IS the fetched remote head, both commit lists empty. Two outcomes were added (`remote_ahead`, `diverged`), and `remote_unavailable` now also covers a failed fetch: a stale cached ref is never trusted, so an offline `forge review-loop` reports `not closeable — remote-unavailable` and exits non-zero rather than trusting the cached ref. The `resolveRemoteRef` candidate order (`@{u}` before `origin/HEAD`) and its rationale are unchanged. Documented in [Reconcile](concepts.md#reconcile), [Show](concepts.md#show), [Report](concepts.md#report), and [Blockers and continuation](concepts.md#blockers-and-continuation).
