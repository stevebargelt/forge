...
1. Author a **candidate** RACI file — start from the effective RACI (your existing `<project>/.forge/forge-raci.md` override, or the host default as a base for a fresh override) — to a scratch path; this is ephemeral working-state, so you write it directly.
2. **Propose** — `forge raci propose <candidate.md> --project <dir> [--json]` (default cwd). This runs the full gate (raci validate → compile → route validate) against the PROJECT override and renders the diff + route-change summary. It **never writes**. A failing gate (unknown agent, non-`human` accountable, weakened host force rule, bad grammar) produces no writable artifact — fix the candidate and re-propose.
...
4. **Apply** — on confirmation, `forge raci apply <candidate.md> --project <dir> --confirm`. Since FG-778 apply targets the **project**: it **re-runs the gate immediately before writing** (never trusts the earlier propose), then writes `<project>/.forge/forge-raci.md`, recompiles `<project>/.forge/routing-policy.yml`, and appends a JSONL line to `<project>/.forge/raci-audit.log` so every routing change is auditable. The host `~/.forge/forge-raci.md` is **never** written by apply (it is forge-owned + always-upgraded since FG-777). A compiled project policy is read directly by dispatch, so the change is **immediately effective for that project — no `forge upgrade` needed**. The gate still refuses a candidate that weakens a host force rule. Without `--confirm`, `apply` behaves like `propose` (dry run).
...
- **Wait completion-driven — a launch-completion wake advances the transition; `ScheduleWakeup` is ONLY a lost-signal watchdog, never a job-duration timer (FG-563 / BD-9).** Sizing a wakeup "matched to how long the work takes" is a defect, not the wait path — the work signals its own completion, and no `sleep N` in Bash ever waits on launched work. The division of labor: **tmux (`forge launch run`) owns the work, `forge launch wait` observes its completion, `forge continue` claims and advances the transition, and `ScheduleWakeup` is only a fixed health-bound backstop for a lost wake.** For every long transition:
  ...
  2. **Arm the wake — it is ENVIRONMENT-DEPENDENT; assume NONE of it is guaranteed.** The UNIVERSAL invariant: `forge launch run` (step 1) owns the work in tmux on every client, and when something re-invokes you, you POLL durable forge state (`forge launch show <id>` / `forge status`) and advance. That poll is the constant; the "wake" only decides WHAT re-invokes you. Use whichever the environment provides, best-to-last-resort:
     - **`Monitor`** — completion-driven, zero-latency: a disposable single-shot `Monitor` running exactly one blocking `forge launch wait <launch-id> --json`, which emits ONE terminal observation as a no-operator-turn wake. It is a DEFERRED tool (load with `ToolSearch("select:Monitor")` before first use) and is **NOT available on Amazon Bedrock / Claude-on-AWS / Google Cloud Agent Platform / Microsoft Foundry** (`code.claude.com/docs/en/tools-reference`). Use it when present; never depend on it.
     - **`ScheduleWakeup`** — a timed self-re-invocation carrying its prompt; the wake when Monitor is absent on a NON-Bedrock client. It is **ALSO unavailable on Bedrock/AWS/GCP/Foundry, and when feature-flag fetching is off** — there a `/loop` with no interval runs on a fixed schedule instead. So do not assume ScheduleWakeup either.
     - **`/loop` fixed-schedule** — on Bedrock/cloud (where neither Monitor nor ScheduleWakeup exists), run the orchestrator under `/loop`; it re-invokes on a fixed cadence, and each tick you poll durable state and advance.
     - **Operator re-prompt** — last resort: end the turn and let the operator's next message drive the poll.

     On Bedrock/cloud there is NO completion-driven wake at all — but no work is lost: `forge launch run` owns it in tmux and forge state is durable, so a `/loop` tick or an operator prompt resumes exactly where you left off. Never a `sleep`/`while` loop, never a blocking foreground wait, and NOT a repeating poll of `forge launch show` held open inside one turn.
  3. **Arm the watchdog (where `ScheduleWakeup` exists):** at most ONE `ScheduleWakeup`, at a FIXED health-bound interval (a lost-signal ceiling), NEVER sized from the job's expected duration. It exists only to recover a wake that never arrived. On a client without `ScheduleWakeup` (Bedrock/cloud), the `/loop` fixed cadence or the operator's next prompt IS the recovery path — there is no `ScheduleWakeup` watchdog to arm.
  4. **End the turn.**
...
- **Waiting on a Forge-owned CI run — register it with `forge ci-wait`, never a bare `gh run watch` / `gh`-polling Monitor (FG-731).** When you need to wait on Forge-owned CI — a PR's required check suite, a push-triggered Actions run, or a `workflow_dispatch` run — arm the wait through `forge ci-wait wait --kind <pr_checks|push_actions|workflow_dispatch> …` (`pr_checks --pr <N>`; `push_actions --sha <headSha>` or `--run-id <id>`; `workflow_dispatch --workflow <file> [--ref <ref>]`; `--repo` defaults to the cwd project's origin). It **durably REGISTERS the wait before it polls**, so it renders on `forge status`/the dashboard as a live WAITING row (kind · state · m/n · elapsed) instead of leaving the workspace looking IDLE while CI grinds off-surface — the exact blank-dashboard gap FG-731 closes. It then block-waits like `forge launch wait`, emitting ONE terminal line, so it drops into the completion-driven pattern above **unchanged**: arm the same single-shot `Monitor` on `forge ci-wait wait … --json`, end the turn, advance on the wake. Terminal outcomes: `completed_awaiting_advance` (CI reached a terminal state — a forge advance/continue is owed, this is your wake to act), `abandoned` (the remote run is gone — merged/closed/deleted), and `timeout`/`cancelled` (WAITER-control outcomes — the record stays live and recovery re-observes it; NOT a CI result). **On a `completed_awaiting_advance` wake, once you have acted on the completed run (advanced/continued the work it gated), record that with `forge ci-wait advance <id>`** — that transitions the wait `completed_awaiting_advance -> advanced` and takes it off the live surface. `completed_awaiting_advance` is a NON-terminal state (a forge advance is owed), so without this the completed wait stays live indefinitely — a forever-live row the FG-590 boundary and this surface's contract forbid. The registration is durable and survives waiter death (recovered by re-observation in reconcile), which is precisely why a bare unregistered `gh` wait is wrong: it is invisible to the operator and leaks no state if the waiter dies. Direct `gh` remains fine for a one-shot read (`gh pr checks`, `gh run view`) — it is the WAIT that must be registered. Stale/terminal launch reaping stays FG-590's concern, not this surface's.
