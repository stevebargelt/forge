...
- **Fix-cycle scope is reconciled against the fixer's OWN declared files, and its refusals are stop-and-read (FG-649, superseding the FG-502 revert note).** The coordinator commits only the paths the batch's results declared — never `git add -A` — and compares that declared set against what actually moved in the worktree. Two outcomes are refusals, NOT FYI: `fix_cycle_declared_changes_absent` (results declare changed files but the tree is clean — the fixer's own ledger contradicts the tree) and `fix_cycle_tree_dirty_outside_declared_scope` (paths moved that no result declared — named, never swept into the commit). Either one means **the fix cycle did NOT complete**: nothing was recorded, the candidate did not move, and the review is still open at Stage 5. Read the named paths, resolve the condition, then re-run `forge review continue` — do not treat it as an informational note and advance past it.
