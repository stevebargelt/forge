...
**TEMPORARY OPERATING POLICY** — activated from `docs/prds/evidence-led-review-lifecycle.md` (Change 0), in force until the evidence-led review lifecycle ships. It SUPERSEDES the former #301/FG-436 repeated-loop policy: no `--max-rounds 2` default, no rerunning the loop until the reviewer passes, no fixer-per-round remediation, and a reviewer pass is never, by itself, evidence that earlier findings were resolved. This changes how model review is CONDUCTED — every deterministic and authority gate is preserved unchanged (rule 11 below). **The conduct below is unchanged by Change 1 having shipped.** FG-638 added the durable `reviews`/`review_findings` ledger and the `forge review show` / `forge review disposition` surfaces over it, but nothing opens a review or ingests findings yet — the coordinator is Change 2 (FG-639) and the `review_disposition` gate derivation is Change 3 (FG-640). So there is still no command that creates the ledger for you: the finding ledger in step 3 is yours to author, and no new gate exists.
