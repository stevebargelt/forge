...
- **`closeable` also requires the reviewed tip to be trusted (FG-502).** The run note and CLI output name the reviewed tip sha and its reachability from the locally cached remote-tracking/PR ref (`git merge-base --is-ancestor`, no implicit fetch). A `✓ closeable` line only prints when that tip is confirmed reachable. If the loop instead prints `✗ not closeable` naming local-only commits, or `remote-unavailable` because no remote-tracking ref resolves, do not close or merge — push the branch (or resolve why no remote ref resolves) and re-run `review-loop`, or re-evaluate, before treating the review as passing.
