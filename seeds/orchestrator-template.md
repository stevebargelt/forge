...
**CI vs review-loop verification — one canonical gate per commit, evidence reused (FG-474).** The review-loop's host verification (typecheck + root `test`, via `runVerification` in `src/v2/review-loop.ts`) is the loop's OWN gate for reviewer/fixer dispatch decisions — but it no longer blindly re-runs when covering evidence already exists: before executing, the loop consults `findCoveringGateEvidence` (src/store/host-verifications.ts) for a passing `host_verifications` row or a green `CI / test` check at the EXACT current HEAD sha, and reuses it (recording what covered it) when the worktree is clean. CI evidence additionally requires content-verified pairing: the TARGET project's own workflow YAML must provably run the exact required gate command (`projectCiRunsCommand` — missing/malformed/non-matching workflow = no CI coverage, fail closed). Campaign reconcile's `runAndRecordHostVerification` does the same — reusing a covering row, or recording a CI-sourced row (source=`ci` + run URL, distinguishable by done-audit) instead of exec'ing the suite. Everything fails closed: no evidence, different sha, different command, pending/failed/disagreeing CI, or a dirty tree → the gate runs for real, exactly as before. The CI check (`CI / test`) remains the required MERGE gate: full aggregate (root + dashboard workspace), off-host, once, visibly, on the pushed branch/PR. Net: one real deterministic run per final commit, consumed as evidence everywhere else.

**Branch protection on `main` — APPLIED (2026-07-08).** `main` requires the `CI / test` check (`enforce_admins` off so operator/orchestrator backlog-only direct pushes still work). This is shared-repo configuration applied from the host under the operator's GitHub session — not something an agent container (no GitHub credentials by design) can or should touch. If it ever needs re-applying: `gh api repos/<owner>/<repo>/branches/main/protection -X PUT` with `required_status_checks.contexts=["test"]` — NOTE the required-check CONTEXT string is the check-run (job) name `test`, not the `CI / test` display form; registering the display form silently never matches and prohibits every merge (hit live 2026-07-08).
