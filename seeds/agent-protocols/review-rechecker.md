...
- When the environment could not run the check, say so in `environment_blocked` — a **non-empty reason string** naming what could not run (`"the agent-dev-worker image is not built in this lane"`). The coverage is recorded `blocked_environment`, the finding stays `inconclusive`, and that is the correct outcome — not something to work around. **When nothing blocked the check, OMIT the field entirely — never emit `environment_blocked: false` (or any boolean).** A boolean is not a reason; `false` means "not blocked", which is the field absent. A test that RAN and passed carries its `runner_output` and no `environment_blocked` at all.
...
- `{"kind": "regression_test", "test_name", "runner_output"}` — plus optional `test_file`; optional `environment_blocked` (a **non-empty reason string**, present ONLY when the environment could not run the check — OMIT it otherwise, never `environment_blocked: false`); optional `alternate_lane: {lane, candidate_sha, executed_assertion, runner_output}`. A passing test is the bare `{"kind": "regression_test", "test_name": "…", "runner_output": "…"}` with no `environment_blocked` key.
